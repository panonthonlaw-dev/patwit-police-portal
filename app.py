...
<style>
    ...
    .metric-value { font-size: 2.2rem; font-weight: 800; color: #1e293b; } .metric-label { font-size: 1rem; color: #64748b; }
</style>
...
states = {
    ...
    'current_user_pwd': "", 'edit_data': None, 'reset_count': 0, 'preserve_search': False
}
...
# ==========================================
# 2. MODULE: INVESTIGATION (โค้ดเดิม 100%)
# ==========================================
...
def investigation_module():
    ...
    with c_nav:
        ...
        if b_home.button("🏠 หน้าหลัก", use_container_width=True, key="inv_home_btn"): setattr(st.session_state, 'current_dept', None); st.rerun()
        if b_logout.button("🚪 ออก", key="inv_logout_btn", use_container_width=True): st.session_state.clear(); st.rerun()
    st.markdown("---")
...
def traffic_module():
    ...
    def upload_to_drive(file_obj, filename):
        file_content = file_obj.getvalue(); base64_str = base64.b64encode(file_content).decode('utf-8'); payload = {"folder_id": DRIVE_FOLDER_ID, "filename": filename, "file": base64_str, "mimeType": file_obj.type}
        try:
            res = requests.post(GAS_APP_URL, json=payload).json(); return res.get("link") if res.get("status") == "success" else None
        except: return None
    def get_img_link(url):
        if not url or pd.isna(url): return None
        match = re.search(r'/d/([a-zA-Z0-9_-]+)|id=([a-zA-Z0-9_-]+)', str(url)); file_id = match.group(1) or match.group(2) if match else None; return f"https://drive.google.com/thumbnail?id={file_id}&sz=w800" if file_id else url
    def create_pdf_tra(vals, img_url1, img_url2, face_url=None, printed_by="ระบบอัตโนมัติ"):
        buffer = io.BytesIO(); c = canvas.Canvas(buffer, pagesize=A4); width, height = A4
        if os.path.exists(FONT_FILE):
            pdfmetrics.registerFont(TTFont('Thai', FONT_FILE)); pdfmetrics.registerFont(TTFont('ThaiBold', FONT_BOLD if os.path.exists(FONT_BOLD) else FONT_FILE)); fn, fb = 'Thai', 'ThaiBold'
        ...
        c.setFont(fb, 22); c.drawCentredString(width/2, height - 50, "แบบทะเบียนประวัติรถจักรยานยนต์นักเรียน"); c.setFont(fn, 18); c.drawCentredString(width/2, height - 72, "โรงเรียนโพนทองพัฒนาวิทยา"); c.line(50, height - 85, width - 50, height - 85)
        name, std_id, classroom, brand, color, plate = str(vals[1]), str(vals[2]), str(vals[3]), str(vals[4]), str(vals[5]), str(vals[6]); lic_s, tax_s, hel_s = str(vals[7]), str(vals[8]), str(vals[9]); raw_note = str(vals[12]).strip() if len(vals) > 12 else ""; note_text = raw_note if raw_note and raw_note.lower() != "nan" else "ไม่พบประวัติ"; score = str(vals[13]) if len(vals) > 13 and str(vals[13]).lower() != "nan" else "100"; c.setFont(fn, 16); c.drawString(60, height - 115, f"ชื่อ-นามสกุล: {name}"); c.drawString(300, height - 115, f"ยี่ห้อรถ: {brand}"); c.drawString(60, height - 135, f"รหัสนักเรียน: {std_id}"); c.drawString(300, height - 135, f"สีรถ: {color}"); c.drawString(60, height - 155, f"ระดับชั้น: {classroom}"); c.setFont(fb, 16); c.drawString(300, height - 155, f"ทะเบียน: {plate}"); c.setFont(fb, 18); color_val = (0.7, 0, 0) if int(score) < 80 else (0, 0.5, 0); c.setFillColorRGB(*color_val); c.drawString(60, height - 185, f"คะแนนความประพฤติจราจรคงเหลือ: {score} คะแนน"); c.setFillColorRGB(0, 0, 0); c.setFont(fn, 16); lm = "(/)" if "มี" in lic_s else "( )"; tm = "(/)" if "ปกติ" in tax_s or "✅" in tax_s else "( )"; hm = "(/)" if "มี" in hel_s else "( )"; c.drawString(60, height - 210, f"สถานะเอกสาร:  {lm} ใบขับขี่    {tm} ภาษี/พรบ.    {hm} หมวกกันน็อค")
        def draw_img(url, x, y, w, h):
            try:
                if url:
                    res = requests.get(get_img_link(url), timeout=5); img_data = ImageReader(io.BytesIO(res.content)); c.drawImage(img_data, x, y, width=w, height=h, preserveAspectRatio=True, mask='auto'); c.rect(x, y, w, h)
            except: pass
    ...
    if st.session_state.traffic_page == 'teacher':
        ...
        st.write(""); c_search, c_btn_search, c_btn_clear = st.columns([3, 1, 1]); q = c_search.text_input("🔍 ค้นหา (ชื่อ/รหัส/ทะเบียน)", key="tra_search_input"); do_search = c_btn_search.button("ค้นหา", type="primary", use_container_width=True); do_clear = c_btn_clear.button("ล้างค่า", type="secondary", use_container_width=True)
        if do_clear: st.session_state.search_results_df = None; st.session_state.df_tra = None; st.rerun()
        st.caption("▼ ตัวกรองข้อมูล (เลือกแล้วกด '⚡ กรองข้อมูล')"); col_f1, col_f2, col_f3 = st.columns(3); unique_lv, unique_br = [], []
        if st.session_state.df_tra is not None: unique_lv = sorted(list(set([str(x).split('/')[0] for x in st.session_state.df_tra.iloc[:, 3].unique()]))); unique_br = sorted(list(set(st.session_state.df_tra.iloc[:, 4].unique())))
        f_risk = col_f1.selectbox("🚨 กลุ่มปัญหา:", ["ทั้งหมด", "❌ ไม่มีใบขับขี่", "❌ ภาษีขาด", "❌ ไม่สวมหมวก"]); f_lv = col_f2.selectbox("📚 ระดับชั้น:", ["ทั้งหมด"] + unique_lv); f_br = col_f3.selectbox("🏍️ ยี่ห้อรถ:", ["ทั้งหมด"] + unique_br); do_filter = st.button("⚡ กรองข้อมูลตามเงื่อนไข", use_container_width=True)
        ...
        if st.session_state.current_user_pwd == "Patwit1510":
            with st.expander("⚙️ ระบบจัดการเลื่อนชั้นเรียน (Super Admin Only)"):
                st.warning("⚠️ คำเตือน:ระบบจะเป็นการแก้ไขถาวรไม่สามารถย้อนกลับได้กรุณาระมัดระวง"); up_pwd = st.text_input("รหัสเลื่อนชั้น", type="password", key="prom_pwd")
        ...
        st.subheader("✏️ แก้ไขข้อมูล"); v = st.session_state.edit_data
        with st.form("edit_form"):
            nm = st.text_input("ชื่อ", v[1]); cl = st.text_input("ชั้น", v[3]); br = st.selectbox("ยี่ห้อ", ["Honda", "Yamaha", "Suzuki", "GPX", "Kawasaki", "อื่นๆ"]); co = st.text_input("สี", v[5]); pl = st.text_input("ทะเบียน", v[6]); lc = st.radio("ใบขับขี่", ["✅ มี", "❌ ไม่มี"], index=0 if "มี" in v[7] else 1, horizontal=True); tx = st.radio("ภาษี", ["✅ ปกติ", "❌ ขาด"], index=0 if "ปกติ" in v[8] or "✅" in v[8] else 1, horizontal=True); hl = st.radio("หมวก", ["✅ มี", "❌ ไม่มี"], index=0 if "มี" in v[9] else 1, horizontal=True); nf = st.file_uploader("เปลี่ยนรูปหลัง"); ns = st.file_uploader("เปลี่ยนรูปข้าง")
        ...
        if st.session_state.df_tra is not None:
            df = st.session_state.df_tra.copy(); df.columns = [f"Col_{i}" for i in range(len(df.columns))] ; c1, c2, c3 = st.columns(3); with c1: st.plotly_chart(px.pie(df, names='Col_7', title="ใบขับขี่", hole=0.3), use_container_width=True); with c2: st.plotly_chart(px.pie(df, names='Col_8', title="ภาษี", hole=0.3), use_container_width=True); with c3: st.plotly_chart(px.pie(df, names='Col_9', title="หมวก", hole=0.3), use_container_width=True); total = len(df); lok = df[df['Col_7'].str.contains("มี", na=False)].shape[0]; m1, m2 = st.columns(2); with m1: st.markdown(f'<div class="metric-card"><div class="metric-value">{total}</div><div class="metric-label">รถทั้งหมด</div></div>', unsafe_allow_html=True); with m2: st.markdown(f'<div class="metric-card"><div class="metric-value">{lok}</div><div class="metric-label">มีใบขับขี่</div></div>', unsafe_allow_html=True)
...
def analytics_module():
    ...
    with st.spinner("⏳ กำลังประมวลผล Big Data รายระดับชั้น..."):
        try:
            ...
            top_inv_risk = df_inv['Incident_Type'].value_counts().idxmax() if 'Incident_Type' in df_inv.columns and not df_inv.empty else "ไม่มีข้อมูล"
            
            # วิเคราะห์เปรียบเทียบระดับชั้น
            if 'ชั้น/ห้อง' in df_tra.columns:
                # รวมจำนวนผิดระเบียบจราจรรายชั้น
                df_tra['Level'] = df_tra['ชั้น/ห้อง'].apply(lambda x: str(x).split('/')[0] if x else "N/A")
                tra_stats = df_tra['Level'].value_counts()

                # รวมจำนวนคดีรายชั้น (สอบสวน) - หากไม่มีคอลัมน์ชั้นในสอบสวน จะใช้ 'Location' หรือ 'Reporter' มาวิเคราะห์เบื้องต้น
                inv_stats = df_inv['Location'].value_counts() # จำลองว่าสถานที่เกิดเหตุบอกกลุ่มเสี่ยง

                # รวมข้อมูลรายชั้น ม.1 - ม.6
                ...
                # กราฟเปรียบเทียบความเสี่ยงรายชั้น
                st.markdown("### 🔍 วิเคราะห์ความเสี่ยงรายระดับชั้น (Grade-Level Analysis)")
                fig = px.bar(comb, x='ชั้น', y=['จราจร', 'สอบสวน'], barmode='group', 
                             title="สถิติการทำผิดระเบียบโรงเรียน (จราจร vs สอบสวน)",
                             labels={'value': 'จำนวนรายการ', 'variable': 'ประเภทงาน'})
                st.plotly_chart(fig, use_container_width=True)

                st.markdown("---")
                c1, c2 = st.columns(2)
                with c1:
                    st.error(f"🚩 **ความเสี่ยงทางคดีสูงสุด:** เรื่อง **{top_inv_risk}**")
                    st.info("💡 **ข้อมูลสรุป:** ระดับชั้นที่พบความถี่การทำผิดระเบียบจราจรสูงสุด จำเป็นต้องมีการปฐมนิเทศเชิงบวก")
                with c2:
                    st.warning("🛡️ **มาตรการเชิงป้องกัน:** แนะนำให้ใช้สายตรวจนักเรียนเฝ้าระวังจุดเกิดเหตุสอบสวนบ่อยครั้งในช่วงพักเที่ยง")

        except Exception as e:
            st.warning("⚠️ ไม่สามารถเข้าถึงฐานข้อมูลจราจรในโหมด Analytics ได้ชั่วคราว กรุณาตรวจสอบสิทธิ์ Secrets")
        ...
        if st.session_state.current_dept is None:
            ...
            with c1:
                with st.container(border=True):
                    ...
                    if st.button("เข้าใช้งานสอบสวน", key="btn_inv_nav", use_container_width=True, type='primary'): st.session_state.current_dept = "inv"; st.rerun()
            with c2:
                with st.container(border=True):
                    st.subheader("🚦 งานจราจร")
                    if st.button("เข้าใช้งานจราจร", key="btn_tra_nav", use_container_width=True, type='primary'): st.session_state.current_dept = "tra"; st.session_state.traffic_page = 'teacher'; st.rerun()
            with c3:
                with st.container(border=True):
                    st.subheader("📊 วิเคราะห์พฤติกรรม")
                    if st.button("เข้าใช้งานระบบ Analytics", key="btn_ana_nav", use_container_width=True, type='primary'): st.session_state.current_dept = "ana"; st.rerun()
        else:
