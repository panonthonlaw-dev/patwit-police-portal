...
def hazard_analytics_module():
    ...
    try:
        ...
        # ตรวจสอบตัวเลขพิกัด (แปลงจากข้อความเป็นตัวเลข)
        df_inv['lat'] = pd.to_numeric(df_inv['lat'], errors='coerce')
        df_inv['lon'] = pd.to_numeric(df_inv['lon'], errors='coerce')
        
        # กรองเฉพาะแถวที่มีพิกัดครบถ้วน (ไม่เอาแถวที่เป็น None)
        df_map = df_inv.dropna(subset=['lat', 'lon'])

        # ... (โค้ดส่วนตรวจสอบข้อมูลด้านบนเหมือนเดิม) ...

        if not df_map.empty:
            st.success(f"✅ ตรวจพบข้อมูลพิกัด {len(df_map)} รายการ กำลังสร้างแผนที่...")
            
            # --- ส่วนที่แก้ไข: ใช้ Folium เพื่อคุมมุมกล้องและขนาดจุด ---
            import folium
            from streamlit_folium import st_folium

            # 1. กำหนดพิกัดกลางโรงเรียน และระดับการซูม (17-18 คือเห็นหลังคาตึกชัด)
            school_center = [16.2941, 103.9782]
            m = folium.Map(location=school_center, zoom_start=18)

            # 2. เลือกแผนที่ภาพถ่ายดาวเทียม (เพื่อให้เห็นตึกจริง)
            folium.TileLayer(
            ...
            # 3. วาดจุดแบบ "วงกลมเล็ก" (CircleMarker) ไม่ให้บังสถานที่
            for index, row in df_map.iterrows():
                # กำหนดสีตามความรุนแรงหรือประเภท (ตัวอย่าง: อุบัติเหตุ=ส้ม, อื่นๆ=แดง)
                color = 'orange' if 'อุบัติเหตุ' in str(row.get('Incident_Type', '')) else '#ef4444'
                
                folium.CircleMarker(
                    location=[row['lat'], row['lon']],
                    radius=6,             # ขนาดวงกลม (เล็กพอดีๆ ไม่บังตึก)
                    color='white',        # สีขอบ
                    weight=1,             # ความหนาขอบ
                    fill=True,
                    fill_color=color,     # สีไส้ใน
                    fill_opacity=0.7,     # ความโปร่งใส (มองทะลุเห็นพื้นหลังได้นิดหน่อย)
                    popup=folium.Popup(f"<b>{row['Location']}</b><br>{row['Incident_Type']}", max_width=200),
                    tooltip=f"{row['Location']}" # เอาเมาส์ชี้แล้วขึ้นชื่อทันที
                ).add_to(m)

            # 4. แสดงผลแผนที่แบบเต็มความกว้าง
            st_folium(m, width="100%", height=500)
            
        else:
            st.warning("📍 เชื่อมต่อสำเร็จ! แต่ยังไม่มีตัวเลขพิกัดใน 5 แถวท้าย (ตามตารางด้านบน)")
