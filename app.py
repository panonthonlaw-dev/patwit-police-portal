...
def create_pdf_inv(row):
    ...
    html_content = f"<html><head><style>@font-face {{ font-family: 'THSarabunNew'; src: url('file://{FONT_FILE}'); }} body {{ font-family: 'THSarabunNew'; font-size: 16pt; }} .header {{ text-align: center; position: relative; min-height: 80px; }} .logo {{ position: absolute; left: 0; width: 60px; }} .box {{ border: 1px solid #000; padding: 10px; min-height: 50px; }} </style></head><body><div class=\"header\">{logo_html}<div style=\"font-size: 22pt; font-weight: bold;\">สถานีตำรวจภูธรโรงเรียนโพนทองพัฒนาวิทยา</div><div>รายงานผลการสอบสวน</div><img style=\"position:absolute; right:0; width:60px;\" src=\"data:image/png;base64,{qr_b64}\"></div><hr><p><b>เลขที่:</b> {rid} | <b>ผู้แจ้ง:</b> {row.get('Reporter','-')}</p><div class=\"box\">{row.get('Statement','-')}</div>{img_html}</body></html>"
...
def investigation_module():
    ...
    try:
        ...
        if st.session_state.view_mode == "list":
            search_q = st.text_input("🔍 ค้นหาคดี", key="inv_search_q")
            ...
            st.write(f"ข้อมูลคดี: {st.session_state.selected_case_id}")
...
# ==========================================
# 4. MODULE: ANALYTICS (ฟังก์ชันวิเคราะห์ใหม่ - กราฟเปรียบเทียบ)
# ==========================================
def analytics_module():
    ...
    with st.spinner("⏳ กำลังวิเคราะห์ข้อมูลพฤติกรรม..."):
        try:
            ...
            if 'ชั้น/ห้อง' in df_tra.columns:
                ...
                inv_lv = df_inv['Location'].value_counts() # จำลองกลุ่มเสี่ยงจากสถานที่
                ...
                st.plotly_chart(px.bar(comb, x='ชั้น', y=['จราจร', 'สอบสวน'], barmode='group', title="สถิติพฤติกรรมผิดระเบียบแยกตามระดับชั้น"), use_container_width=True)
        except: st.warning("กำลังรวบรวมข้อมูลสถิติ...")
