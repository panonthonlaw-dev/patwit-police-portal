...
# --- PDF Libraries Setup ---
# 1. WeasyPrint สำหรับงานสอบสวน (HTML PDF)
try:
    from weasyprint import HTML, CSS
    from weasyprint.text.fonts import FontConfiguration
except: pass

# 2. ReportLab สำหรับงานจราจร (Canvas PDF)
...
def process_image(img_file):
    ...
    try:
        img = Image.open(img_file); img = img.convert('RGB'); img.thumbnail((800, 800))
        buf = io.BytesIO(); img.save(buf, format="JPEG", quality=65, optimize=True)
        return base64.b64encode(buf.getvalue()).decode()
    except: return ""

# ==========================================
# 2. MODULE: INVESTIGATION (กู้คืน PDF HTML + ลายเซ็น 5 คน)
# ==========================================
def create_pdf_inv_original(row):
    # กู้คืน Logic PDF แบบ HTML ที่มีลายเซ็น 5 ช่อง
    ...
    # จัดการรูปภาพ (แสดงทั้งรูปผู้แจ้ง และ รูปหลักฐานเพิ่ม)
    img_html = ""
    if clean_val(row.get('Image_Data')):
        img_html += f"<div style='text-align:center;margin-top:5px;'><b>ภาพเหตุการณ์:</b><br><img src='data:image/jpeg;base64,{row.get('Image_Data')}' style='max-width:350px;max-height:200px;border:1px solid #ccc;'></div>"
    if clean_val(row.get('Evidence_Image')):
        img_html += f"<div style='text-align:center;margin-top:5px;'><b>หลักฐานเพิ่มเติม:</b><br><img src='data:image/jpeg;base64,{row.get('Evidence_Image')}' style='max-width:350px;max-height:200px;border:1px solid #ccc;'></div>"
    ...
    <html><head><style>
        ...
        .header {{ text-align: center; position: relative; min-height: 90px; }}
        .logo {{ position: absolute; top: 0; left: 0; width: 60px; }}
        .qr {{ position: absolute; top: 0; right: 0; width: 60px; }}
        .box {{ border: 1px solid #000; background-color: #f9f9f9; padding: 8px; min-height: 60px; white-space: pre-wrap; }}
        .sig-table {{ width: 100%; margin-top: 20px; text-align: center; border-collapse: collapse; }}
    ...
    <body>
        ...
        <table style="width:100%;">
            <tr><td width="60%"><b>เลขที่รับแจ้ง:</b> {rid}</td><td width="40%" style="text-align:right;"><b>วันที่แจ้ง:</b> {date_str}<br><b>วันที่บันทึกผล:</b> {latest_date}</td></tr>
        </table>
        <p><b>ผู้แจ้ง:</b> {row.get('Reporter','-')} | <b>ประเภทเหตุ:</b> {row.get('Incident_Type','-')} | <b>สถานที่:</b> {row.get('Location','-')}</p>
        <div><b>รายละเอียดเหตุการณ์:</b></div><div class="box">{row.get('Details','-')}</div>
        <div style="margin-top:5px;"><b>ผลการดำเนินการสอบสวน:</b></div><div class="box">{row.get('Statement','-')}</div>
        {img_html}
    ...
    try:
        ...
        if st.session_state.view_mode == "list":
            ...
            with tab_list:
                ...
                tot_p = math.ceil(len(df_p)/5) or 1
                if st.session_state.page_pending > tot_p: st.session_state.page_pending = 1
                start_p = (st.session_state.page_pending - 1) * 5
                
                if df_p.empty: st.caption("ไม่มีรายการ")
                for i, row in df_p.iloc[start_p : start_p+5].iterrows():
                    ...
                    cc2.write(row['Timestamp']); cc3.write(row['Incident_Type']); cc4.markdown(f"<span style='color:orange;'>{row['Status']}</span>", unsafe_allow_html=True); st.divider()
                ...
                tot_f = math.ceil(len(df_f)/5) or 1
                if st.session_state.page_finished > tot_f: st.session_state.page_finished = 1
                start_f = (st.session_state.page_finished - 1) * 5
                
                for i, row in df_f.iloc[start_f : start_f+5].iterrows():
                    cc1, cc2, cc3, cc4 = st.columns([2.5, 2, 3, 1.5])
            ...
            if not sel.empty:
                ...
                st.divider()
                # เรียกใช้ create_pdf_inv_original ที่กู้คืนมา
                try:
                    pdf_data = create_pdf_inv_original(row)
                ...
                except: st.error("PDF ขัดข้อง (ตรวจสอบ Library WeasyPrint)")
    except Exception as e: st.error(f"Error: {e}")

# ==========================================
# 3. MODULE: TRAFFIC (เจ้าหน้าที่ล้วน - แก้ Syntax & Load Data)
# ==========================================
def traffic_module():
