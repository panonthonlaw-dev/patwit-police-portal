...
def hazard_analytics_module():
    ...
    try:
        # 1. เช็กว่าแอปกำลังอ่านชีตชื่ออะไร
        target_sheet = get_target_sheet_name()
        st.write(f"🔍 ขณะนี้แอปกำลังพยายามอ่านจากชีตชื่อ: `{target_sheet}`")

        # 2. อ่านข้อมูลแบบดิบที่สุด (ไม่ง้อ Cache)
        df = conn.read(worksheet=target_sheet, ttl=0)
        
        # 3. แสดงชื่อคอลัมน์ทั้งหมดที่แอป "เห็น" จริงๆ
        st.write("📋 ชื่อคอลัมน์ที่ระบบตรวจพบในไฟล์นี้คือ:")
        st.code(df.columns.tolist())

        # 4. ตรวจสอบเงื่อนไขพิกัด
        ...
        if 'lat' in df.columns and 'lon' in df.columns:
            ...
            if not df_map.empty:
                st.success(f"✅ ตรวจพบพิกัด {len(df_map)} รายการ!")
                st.map(df_map[['lat', 'lon']])
            else:
                st.warning("📍 พบคอลัมน์แล้ว แต่ข้อมูลในช่อง lat/lon เป็นค่าว่าง หรือไม่ใช่ตัวเลข")
        else:
            st.error("❌ ระบบยังมองไม่เห็นคอลัมน์ 'lat' หรือ 'lon'")
