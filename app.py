    ...
    try:
        ...
        if st.session_state.view_mode == "list":
            ...
            with tab_list:
                ...
                h1.markdown("**เลขที่รับแจ้ง**")
                h2.markdown("**วันเวลา**")
                h3.markdown("**ประเภทเหตุ**")
                h4.markdown("**สถานะ**")
                ...
                for i, row in df_p.iloc[start_p:end_p].iterrows():
                    ...
                    with cc1: 
                        st.button(f"📝 {row['Report_ID']}", key=f"p_{i}", use_container_width=True, on_click=lambda r=row['Report_ID']: st.session_state.update({'selected_case_id': r, 'view_mode': 'detail', 'unlock_password': ""}))
                ...
                if tot_p > 1:
                    ...
                    if cp1.button("⬅️ ย้อนกลับ", disabled=st.session_state.page_pending==1, key="pp"): 
                    ...
                    if cp3.button("ถัดไป ➡️", disabled=st.session_state.page_pending==tot_p, key="pn"): 
                        st.session_state.page_pending+=1
                        st.rerun()
                ...
                for i, row in df_f.iloc[start_f:end_f].iterrows():
                    cc1, cc2, cc3, cc4 = st.columns([2.5, 2, 3, 1.5])
                    with cc1: 
                        st.button(f"✅ {row['Report_ID']}", key=f"f_{i}", use_container_width=True, on_click=lambda r=row['Report_ID']: st.session_state.update({'selected_case_id': r, 'view_mode': 'detail', 'unlock_password': ""}))
                    cc2.write(row['Timestamp'])
                    cc3.write(row['Incident_Type'])
                ...
                if tc > 0:
                    ...
                    with c_text1:
                        ...
                        for l, c in df_display['Location'].value_counts().head(5).items(): 
                            p = (c/tc)*100
                            st.markdown(f"- **{l}**: {c} ครั้ง <span style='color:red; font-size:0.8em;'>({p:.1f}%)</span>", unsafe_allow_html=True)
                    with c_text2:
                        st.markdown("**📌 สรุปยอดตามประเภทเหตุ**")
                        for t, c in df_display['Incident_Type'].value_counts().head(5).items(): 
                            p = (c/tc)*100
                            st.markdown(f"- **{t}**: {c} ครั้ง <span style='color:red; font-size:0.8em;'>({p:.1f}%)</span>", unsafe_allow_html=True)
                    st.divider()
            ...
            if not sel.empty:
                ...
                with st.form("full_inv_form"):
                    ...
                    if st.form_submit_button("💾 บันทึกข้อมูล") and not is_lock:
                        ...
                        if ev_img: 
                            df_raw.at[idx_raw, 'Evidence_Image'] = process_image(ev_img)
                ...
                if clean_val(row['Audit_Log']):
                    with st.expander("📜 ประวัติการบันทึก (Audit Log)"): 
                        st.code(row['Audit_Log'])
                st.divider()
                try:
...
def traffic_module():
    ...
    def create_pdf_tra(vals, img_url1, img_url2, face_url=None, printed_by="ระบบอัตโนมัติ"):
        ...
        c.drawString(300, height - 115, f"ยี่ห้อรถ: {brand}")
        c.drawString(60, height - 135, f"รหัสนักเรียน: {std_id}")
        c.drawString(300, height - 135, f"สีรถ: {color}")
        c.drawString(60, height - 155, f"ระดับชั้น: {classroom}")
        c.setFont(fb, 16)
        c.drawString(300, height - 155, f"ทะเบียน: {plate}")
        c.setFont(fb, 18)
        ...
        c.setFont(fn, 16)
        lm = "(/)" if "มี" in lic_s else "( )"
        tm = "(/)" if "ปกติ" in tax_s or "✅" in tax_s else "( )"
        hm = "(/)" if "มี" in hel_s else "( )"
        c.drawString(60, height - 210, f"สถานะเอกสาร:  {lm} ใบขับขี่    {tm} ภาษี/พรบ.    {hm} หมวกกันน็อค")
        def draw_img(url, x, y, w, h):
            try:
                if url:
                    res = requests.get(get_img_link(url), timeout=5)
                    img_data = ImageReader(io.BytesIO(res.content))
                    c.drawImage(img_data, x, y, width=w, height=h, preserveAspectRatio=True, mask='auto')
                    c.rect(x, y, w, h)
            except: pass
        draw_img(vals[10], 70, height - 415, 180, 180)
        draw_img(vals[11], 300, height - 415, 180, 180)
        note_y = height - 455
        c.setFont(fb, 16)
        c.drawString(60, note_y, "ประวัติบันทึกการทำผิดวินัยจราจร:")
        c.setFont(fn, 15)
        text_obj = c.beginText(70, note_y - 25)
        text_obj.setLeading(20)
        for line in note_text.split('\n'):
            for w_line in textwrap.wrap(line, width=75): 
    ...
    if st.session_state.traffic_page == 'teacher':
                ...
                for i, row in target_df.iterrows():
                    ...
                    with st.expander(f"📍 {v[6]} | {v[1]}"):
                        ...
                        if st.session_state.officer_role == "admin":
                            ...
                            col_act1.download_button("📥 โหลด PDF", create_pdf_tra(v, v[10], v[11], v[14], st.session_state.officer_name), f"{v[6]}.pdf", key=f"pdf_{i}", use_container_width=True)
                            if col_act2.button("✏️ แก้ไขข้อมูล", key=f"ed_{i}", use_container_width=True): 
        ...
        if st.session_state.current_user_pwd == "Patwit1510":
            with st.expander("⚙️ ระบบจัดการเลื่อนชั้นเรียน (Super Admin Only)"):
                st.warning("⚠️ คำเตือน:ระบบจะเป็นการแก้ไขถาวรไม่สามารถย้อนกลับได้")
        ...
        with st.form("edit_form"):
            nm = st.text_input("ชื่อ", v[1])
            cl = st.text_input("ชั้น", v[3])
            br = st.selectbox("ยี่ห้อ", ["Honda", "Yamaha", "Suzuki", "GPX", "Kawasaki", "อื่นๆ"])
            co = st.text_input("สี", v[5])
            pl = st.text_input("ทะเบียน", v[6])
            lc = st.radio("ใบขับขี่", ["✅ มี", "❌ ไม่มี"], index=0 if "มี" in v[7] else 1, horizontal=True)
            tx = st.radio("ภาษี", ["✅ ปกติ", "❌ ขาด"], index=0 if "ปกติ" in v[8] or "✅" in v[8] else 1, horizontal=True)
            hl = st.radio("หมวก", ["✅ มี", "❌ ไม่มี"], index=0 if "มี" in v[9] else 1, horizontal=True)
            nf = st.file_uploader("เปลี่ยนรูปหลัง")
            ns = st.file_uploader("เปลี่ยนรูปข้าง")
...
# ==========================================
# 4. MODULE: ANALYTICS (ฟังก์ชันใหม่)
# ==========================================
def analytics_module():
    ...
    with c_nav:
        ...
        st.write("")
        b_home, b_logout = st.columns(2)
        if b_home.button("🏠 หน้าหลัก", key="ana_h"): 
            setattr(st.session_state, 'current_dept', None)
            st.rerun()
        if b_logout.button("🚪 ออก", key="ana_o"): 
            st.session_state.clear()
            st.rerun()
    st.markdown("---")
    ...
    with st.spinner("⏳ กำลังวิเคราะห์ข้อมูลรายระดับชั้น..."):
        try:
            ...
            st.warning("ระบบวิเคราะห์กำลังดึงข้อมูลฐานข้อมูลจราจร...")
        ...
        if st.session_state.current_dept is None:
            ...
            with c3:
                with st.container(border=True):
                    ...
                    if st.button("เข้าใช้งานระบบ Analytics", key="btn_ana", use_container_width=True, type='primary'): 
                        st.session_state.current_dept = "ana"
                        st.rerun()
        else:
