...
def create_hazard_map_obj(_df):
    ...
    # 1. กรองข้อมูลเบื้องต้น (เอาเฉพาะที่มีพิกัด และไม่ใช่ "อื่นๆ")
    active_locs = [loc for loc in COORD_MAP.keys() if loc != "อื่นๆ"]
    df_filtered = _df[_df['Location'].isin(active_locs)].copy()
    
    # 2. นับจำนวนเหตุการณ์รายสถานที่ (สร้างเป็น Dictionary)
    # ตัวแปรนี้จะบอกว่า อาคาร 1 มีกี่เคส, อาคาร 2 มีกี่เคส
    risk_counts = df_filtered['Location'].value_counts().to_dict()
    
    # 3. กำหนดจุดกึ่งกลาง (สตรีศึกษา)
    ...
    for _, row in df_filtered.iterrows():
        ...
        # ดึงจำนวนครั้งที่เกิดเหตุในที่นี้มาตัดสินใจเรื่องสี
        count = risk_counts.get(loc_name, 0)
        
        # --- เกณฑ์การเปลี่ยนสี (ปรับให้เห็นง่ายที่สุด) ---
        if count >= 3:
            marker_color = '#dc2626'  # แดง (3 ครั้งขึ้นไป)
        elif count == 2:
            marker_color = '#facc15'  # เหลือง (2 ครั้ง)
        else:
            marker_color = '#22c55e'  # เขียว (1 ครั้ง)
            
        # Jitter ป้องกันหมุดซ้อนกันในอาคารเดียว
        ...
        folium.CircleMarker(
            ...
            fill_opacity=1.0,
            popup=folium.Popup(f"📍 <b>{loc_name}</b><br>สถิติสะสม: {count} ครั้ง", max_width=200),
            tooltip=f"{loc_name}: {count} ครั้ง"
        ).add_to(cluster)
...
def hazard_analytics_module():
    ...
    try:
        ...
        # ✅ สำคัญ: ใช้ ttl=0 ชั่วคราวเพื่อทดสอบว่าสีเปลี่ยนไหม 
        # (ถ้าสีเปลี่ยนแล้วค่อยปรับกลับเป็น 10800 เพื่อประหยัดโควต้าครับ)
        conn = st.connection("gsheets", type=GSheetsConnection)
        ...
        if not df_inv.empty:
            ...
            # สร้างแผนที่ใหม่ทุกครั้งที่โหลดหน้านี้ (ห้ามใส่ @st.cache_resource ที่ฟังก์ชันสร้างแผนที่)
            ...
            if map_obj:
                st_folium(
                    ...
                    width="100%", 
                    height=450, 
                    key=f"hazard_map_final_{target_sheet}", # key ต้องเปลี่ยนตามชื่อชีต
                    returned_objects=[], 
                    use_container_width=True
                )
            
            # ปุ่มล้างแคชข้อมูล
            if st.button("🔄 รีเฟรชข้อมูลล่าสุดจาก Google Sheets"):
                st.cache_data.clear()
                st.rerun()
        else:
