...
def create_summon_pdf(row, appointment_info):
    ...
    <html><head><style>
        ...
        <p style="margin-top: 20px;">จึงเรียนมาเพื่อโปรดปฏิบัติหน้าที่และมาพบตำรวจนักเรียนตามกำหนดเวลา</p>
...
# --- [ส่วนที่ 1: ระบบเช็คสิทธิ์แก้ไข] ---
                ...
                # ==========================================
                # ⚖️ [ระบบออกหมายเรียกแบบระบุวันเวลานัด]
                # ==========================================
                ...
                if user_role in ["admin", "super_admin"]:
                    if case_status == "อยู่ระหว่างการดำเนินการ":
                        ...
                        # 1. เพิ่มช่องกรอกวันเวลานัดหมาย (จะปรากฏขึ้นมาเมื่อสถานะถูกต้อง)
                        appt_val = st.text_input(
                            "📅 ระบุวันและเวลานัดหมายให้ชัดเจน:", 
                            placeholder="เช่น วันจันทร์ที่ 5 ม.ค. 2569 เวลา 09.00 น.",
                            key=f"input_appt_{sid}"
                        )
                        
                        # 2. ถ้ากรอกข้อมูลแล้ว ถึงจะให้กดโหลด PDF
                        if appt_val:
                            try:
                                # ส่งค่า appt_val เข้าไปในฟังก์ชันสร้าง PDF
                                summon_pdf = create_summon_pdf(row, appt_val)
                                
                                st.download_button(
                                    label=f"📥 ดาวน์โหลดหมายเรียก (นัดหมาย: {appt_val})",
                                    data=summon_pdf,
                            ...
                            st.warning("⚠️ โปรดพิมพ์ 'วันและเวลานัดหมาย' ในช่องด้านบนก่อน เพื่อสร้างหมายเรียก")
