...
def create_hazard_map_obj(_df):
    ...
    # 1. นับความถี่รายอาคารเพื่อจัดระดับความเสี่ยง
    ...
    # 3. วนลูปปักหมุดตามรายชื่ออาคาร (ข้าม "อื่นๆ")
    for loc_name, coords in COORD_MAP.items():
        ...
        folium.CircleMarker(
            ...
            fill_opacity=1.0,    # ✅ ปรับให้สีทึบ 100% ไม่โปร่งแสง
            popup=f"📍 <b>{loc_name}</b><br>สถิติสะสม: {count} ครั้ง",
            tooltip=f"{loc_name}: {count} ครั้ง"
        ).add_to(m) # ✅ ปักลงแผนที่โดยตรง (ไม่ผ่าน Cluster) เพื่อให้สีไม่เพี้ยน
