...
# --- ส่วนที่แก้ไข: ระบบล็อกตามระดับสิทธิ์ 3 ระดับ ---
                ...
                if user_role == "super_admin":
                    if cur_sta == "ดำเนินการเรียบร้อย":
                        ...
                        is_lock = False # คดียังไม่จบ Super Admin แก้ได้เลย
                
                elif user_role == "admin":
                    if cur_sta != "ดำเนินการเรียบร้อย":
                        is_lock = False # ครูทั่วไปแก้ได้เฉพาะคดีที่ยังไม่ปิด
                    else:
                        st.warning("🔒 เฉพาะแอดมินสูงสุดเท่านั้นที่แก้ไขคดีที่จบแล้วได้")
                        is_lock = True
                
                else: # Viewer (ตำรวจนักเรียน)
