...
def hazard_analytics_module():
    ...
    try:
        # 1. บังคับอ่านใหม่ (ttl=0)
        target_sheet = get_target_sheet_name()
        df_inv = conn.read(worksheet=target_sheet, ttl=0)
        
        # 2. ตรวจสอบชื่อคอลัมน์ที่ระบบอ่านได้จริง (สำหรับ Debug)
        actual_columns = df_inv.columns.tolist()
        
        # 3. ตรวจสอบว่ามี lat และ lon หรือไม่
        if 'lat' not in actual_columns or 'lon' not in actual_columns:
            st.error(f"⚠️ ระบบยังไม่พบคอลัมน์ 'lat' หรือ 'lon'")
            st.write("คอลัมน์ที่ระบบตรวจพบในปัจจุบัน:", actual_columns)
            st.info("💡 คำแนะนำ: ตรวจสอบที่ Google Sheet ช่อง R1 ต้องพิมพ์ 'lat' และ S1 ต้องพิมพ์ 'lon' (ห้ามมีช่องว่าง)")
            return

        # 4. แปลงค่าและกรองข้อมูล
        ...
        if df_map.empty:
            st.warning("📍 พบคอลัมน์แล้ว แต่ไม่มี 'ตัวเลขพิกัด' ในแถวข้อมูลเลย (กรุณาแจ้งเหตุและกดปุ่ม GPS ก่อน)")
            return
