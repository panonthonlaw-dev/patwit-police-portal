...
# --- [ส่วนที่เพิ่ม: การ์ดสถิติสรุปภาพรวม (Metric Cards)] ---
        ...
        if st.session_state.view_mode == "list":
            ...
            if not sel.empty:
                ...
                # --- [ส่วนที่แก้ไข: ระบบล็อกและปลดล็อกตามสิทธิ์ 3 ระดับ] ---
                cur_sta = clean_val(row['Status'])
                user_role = st.session_state.user_info.get('role')

                # 1. กำหนดค่าเริ่มต้น: ล็อกไว้ก่อนเพื่อความปลอดภัย
                is_lock = True 

                # 2. กรณีเป็น Super Admin (มีสิทธิ์สูงสุด)
                if user_role == "super_admin":
                    if cur_sta == "ดำเนินการเรียบร้อย":
                        # ถ้าคดีจบแล้ว ต้องกดปุ่มยืนยันก่อน (ล็อก 2 ชั้นป้องกันมือลั่น)
                        ...
                        # ถ้าคดียังไม่จบ Super Admin แก้ไขได้ทันที
                        is_lock = False

                # 3. กรณีเป็น Admin ทั่วไป (เช่น คุณครูปกครอง)
                elif user_role == "admin":
                    if cur_sta != "ดำเนินการเรียบร้อย":
                        # แก้ไขได้เฉพาะคดีที่ยังไม่ปิดสถานะ
                        is_lock = False 
                    else:
                        # ถ้าปิดคดีไปแล้ว Admin ทั่วไปจะถูกล็อกถาวร (ต้องให้ Super Admin มาแก้ให้)
                        st.warning("🔒 เคสนี้ดำเนินการเรียบร้อยแล้ว เฉพาะแอดมินสูงสุดเท่านั้นที่แก้ไขได้")
                        is_lock = True

                # 4. กรณีเป็น Viewer (เช่น ตำรวจนักเรียน)
                else:
                    # is_lock จะเป็น True เสมอ (ดูได้อย่างเดียว)
...
def traffic_module():
    ...
    if st.session_state.traffic_page == 'teacher':
       ...
       # เปลี่ยนมาเช็ค Role 'super_admin' แทนการเช็ครหัสผ่านตรงๆ
if st.session_state.user_info.get("role") == "super_admin":
    ...
    if up_pwd == UPGRADE_PASSWORD: # ระบบจะไปดึง "Patwitsafe" มาเทียบ
