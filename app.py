...
# --- ส่วนเพิ่มปุ่มใน Sidebar หรือ Menu (เพื่อให้แยกจากโครงสร้างเดิม) ---
if st.sidebar.button("🖥️ ศูนย์เฝ้าระวังเหตุ (Real-time Monitor)", use_container_width=True):
    st.session_state.page = 'monitor_center'
    st.rerun()

# --- หน้าศูนย์เฝ้าระวังเหตุการณ์ ---
if st.session_state.get('page') == 'monitor_center':
    # ปุ่มกลับหน้าหลัก
    if st.button("⬅️ ปิดหน้าจอมอนิเตอร์", use_container_width=True):
        st.session_state.page = 'main'; st.rerun()

    # หัวข้อ Command Center
    st.markdown("""
        <div style="background-color:#0f172a; padding:20px; border-radius:10px; border-left:10px solid #ef4444; margin-bottom:20px;">
            <h1 style="color:white; margin:0;">🚨 LIVE INCIDENT MONITOR</h1>
            <p style="color:#94a3b8; margin:0;">สถานีตำรวจนักเรียนโพนทองพัฒนาวิทยา | Command Center</p>
        </div>
    """, unsafe_allow_html=True)

    # ดึงข้อมูลล่าสุด (สมมติใช้ df_inv จากระบบสอบสวน)
    if 'df_inv' in st.session_state and st.session_state.df_inv is not None:
        df = st.session_state.df_inv.copy()
        
        # กรองข้อมูล 5 รายการล่าสุดที่ยังไม่เสร็จสิ้น หรือรายการล่าสุดทั้งหมด
        latest_incidents = df.tail(10).iloc[::-1] # ดึง 10 เคสล่าสุด สลับเอาใหม่สุดขึ้นก่อน

        # ส่วนแสดงผล Stats Card (แถวบน)
        col1, col2, col3, col4 = st.columns(4)
        with col1:
            st.metric("เหตุการณ์วันนี้", len(latest_incidents))
        with col2:
            st.metric("รอดำเนินการ", (df['C11'] == 'รอดำเนินการ').sum(), delta_color="inverse")
        with col3:
            st.metric("กำลังตรวจเช็ค", (df['C11'] == 'กำลังดำเนินการ').sum())
        with col4:
            st.metric("เหตุปกติ", (df['C11'] == 'เสร็จสิ้น').sum())

        st.write("---")

        # ส่วนแสดงรายการเหตุการณ์แบบ Real-time Monitor
        st.markdown("### 📡 รายการแจ้งเหตุล่าสุด")
        
        for index, row in latest_incidents.iterrows():
            # กำหนดสีตามความด่วน (อิงจากคอลัมน์สถานะ C11 หรือประเภทเหตุ C5)
            status_color = "#ef4444" if row['C11'] == 'รอดำเนินการ' else "#f59e0b" if row['C11'] == 'กำลังดำเนินการ' else "#10b981"
            
            # HTML Card สำหรับแต่ละเหตุการณ์
            st.markdown(f"""
                <div style="background-color: #1e293b; color: white; padding: 15px; border-radius: 8px; margin-bottom: 10px; border-left: 5px solid {status_color};">
                    <div style="display: flex; justify-content: space-between; align-items: start;">
                        <div>
                            ...
                            <span style="margin-left:10px; font-weight:bold; font-size:18px;">📍 {row['C4'] if 'C4' in row else 'ไม่ระบุสถานที่'}</span>
                        </div>
                        <div style="color:#94a3b8; font-size:12px;">⏰ {row['C1'] if 'C1' in row else 'เพิ่งแจ้ง'}</div>
                    </div>
                    <div style="margin-top: 10px;">
                        <b style="color:#cbd5e1;">ประเภทเหตุ:</b> {row['C5']} | <b style="color:#cbd5e1;">ผู้เกี่ยวข้อง:</b> {row['C2']}
                    </div>
                    <div style="margin-top: 5px; font-size: 14px; color: #94a3b8;">
                        📝 <b>รายละเอียด:</b> {row['C6'] if 'C6' in row else 'ไม่มีข้อมูลเพิ่มเติม'}
                    </div>
                </div>
            """, unsafe_allow_html=True)

        # เพิ่มปุ่ม Refresh ข้อมูลแบบแมนนวล (หรือจะใช้ st.empty สำหรับ auto-refresh)
        if st.button("🔄 อัปเดตข้อมูลสด"):
            st.rerun()

    else:
        st.warning("📭 ยังไม่มีข้อมูลเหตุการณ์ในระบบ")
