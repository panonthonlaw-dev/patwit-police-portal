...
def create_hazard_map_obj(_df):
    ...
    # วนลูปตามรายชื่ออาคารที่กำหนดไว้ใน COORD_MAP
    for loc_name, coords in COORD_MAP.items():
        if loc_name == "อื่นๆ": continue
        
        count = risk_counts.get(loc_name, 0)
        if count == 0: continue # ถ้าไม่มีเหตุไม่ต้องปักหมุด
        
        # กำหนดสีตามเกณฑ์: สูง(3+) = แดง, กลาง(2) = เหลือง, ต่ำ(1) = เขียว
        if count >= 3:
            marker_color = '#dc2626'
        elif count == 2:
            marker_color = '#facc15'
        ...
        # ปักหมุดแบบวงกลมคงที่เพื่อให้เห็นระดับสีชัดเจน
        folium.Circle(
            ...
            fill_opacity=0.6,
            popup=f"📍 {loc_name}<br>จำนวนเหตุ: {count} ครั้ง",
