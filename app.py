...
# ฟังก์ชันเชื่อมต่อฐานข้อมูลจราจร (นำมาไว้ส่วนกลาง)
...
# ==========================================
# 4. MODULE: ANALYTICS (ระบบวิเคราะห์เชิงลึก - Read Only)
# ==========================================
    ...
    with st.spinner("⏳ ประมวลผล Big Data เปรียบเทียบระหว่างแผนก..."):
        try:
            ...
            if 'ชั้น/ห้อง' in df_t.columns:
                ...
                i_count = df_i['Location'].value_counts() # จำลองกลุ่มเสี่ยง
                
                comb = pd.DataFrame({'งานจราจร': t_count, 'งานสอบสวน': i_count}).fillna(0).reset_index().rename(columns={'index': 'ชั้น'})
        ...
        except: st.warning("ระบบกำลังบูรณาการฐานข้อมูลเพื่อทำการวิเคราะห์เชิงลึก...")

# ==========================================
# 5. MAIN ENTRY (แก้ไข Syntax Error ทั้งหมด)
# ==========================================
def main():
