...
def hazard_analytics_module():
    ...
    try:
        ...
        # ✅ อ่านข้อมูลสด (ttl=0) และบังคับเป็น DataFrame
        df_raw = conn.read(worksheet=target_sheet, ttl=0)
        df_inv = pd.DataFrame(df_raw)

        # 🛠️ จุดตาย: บังคับล้างชื่อคอลัมน์ทุกตัวให้สะอาด (ตัดช่องว่างทิ้ง)
        df_inv.columns = [str(c).strip() for c in df_inv.columns]

        # 🛠️ จุดตาย 2: ถ้ายังหาไม่เจอ ให้แอป "สร้างคอลัมน์หลอก" ขึ้นมาเองก่อนเพื่อไม่ให้เด้งไป Except
        if 'lat' not in df_inv.columns: df_inv['lat'] = ""
        if 'lon' not in df_inv.columns: df_inv['lon'] = ""

        # --- ส่วนแสดงผลตารางเช็กข้อมูล ---
        st.write("### 🔍 ตรวจสอบพิกัดล่าสุด (จาก Google Sheet)")
        
        # เลือกแสดงเฉพาะที่จำเป็น
        cols_to_show = [c for c in ['Report_ID', 'Location', 'lat', 'lon'] if c in df_inv.columns]
        st.dataframe(df_inv[cols_to_show].tail(10), use_container_width=True)

        # ตรวจสอบว่ามีตัวเลขพิกัดโผล่มาบ้างหรือยัง
        df_inv['lat_num'] = pd.to_numeric(df_inv['lat'], errors='coerce')
        if df_inv['lat_num'].isnull().all():
            st.warning("📍 ระบบเชื่อมต่อคอลัมน์ได้แล้ว แต่ 'ในตารางด้านบน' ยังไม่มีตัวเลขพิกัดโผล่มาเลย")
            st.info("💡 วิธีแก้: ให้คุณไปที่ Google Sheet (ช่อง R2, S2) แล้วพิมพ์ตัวเลข 16.2941 และ 103.9782 ลงไปตรงๆ เพื่อทดสอบครับ")
