...
def hazard_analytics_module():
    ...
    st.markdown("### 🔍 ระบบตรวจสอบการเชื่อมต่อพิกัด (Debug Console)")

    try:
        # 1. เช็กชื่อ Sheet ที่แอปพยายามเข้าถึง
        target_sheet = get_target_sheet_name()
        st.info(f"ขณะนี้แอปกำลังดึงข้อมูลจาก Worksheet ชื่อ: `{target_sheet}`")

        # 2. บังคับอ่านข้อมูลแบบ RAW (ห้ามใช้ Cache)
        df = conn.read(worksheet=target_sheet, ttl=0)
        
        # 3. ตรวจสอบชื่อคอลัมน์ทั้งหมด (จะโชว์ออกมาเป็นลิสต์)
        st.write("📋 รายชื่อคอลัมน์ที่ระบบ 'มองเห็น' ใน Google Sheet ของคุณ:")
        st.code(df.columns.tolist())

        # 4. ตรวจสอบข้อมูล 5 แถวล่าสุด (ดูว่า lat/lon ที่กรอกมือโผล่มาไหม)
        st.write("📊 ข้อมูล 5 แถวล่าสุดที่ระบบดึงมาได้:")
        ...
        if 'lat' in df.columns and 'lon' in df.columns:
            ...
            if not df_map.empty:
                st.success(f"✅ พบข้อมูลที่มีพิกัดถูกต้อง {len(df_map)} รายการ")
                st.map(df_map[['lat', 'lon']])
            else:
                st.warning("📍 ระบบพบคอลัมน์ lat/lon แล้ว แต่ค่าที่กรอกมาไม่ใช่ตัวเลข หรือเป็นค่าว่าง")
        else:
            st.error("❌ ไม่พบคอลัมน์ 'lat' และ 'lon' ใน DataFrame นี้")
