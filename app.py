...
# Universal Connector (ย้ายมาไว้ข้างนอกเพื่อให้ Analytics เรียกใช้ได้)
...
def create_pdf_inv(row):
    ...
    if clean_val(row.get('Evidence_Image')):
        img_html += f"<div style='text-align:center;'><img src='data:image/jpeg;base64,{row.get('Evidence_Image')}' style='max-width:380px;'></div>"
    logo_html = f'<img class="logo" src="data:image/png;base64,{LOGO_BASE64}">' if LOGO_BASE64 else ""
    html_content = f"<html><head><style>@font-face {{ font-family: 'THSarabunNew'; src: url('file://{FONT_FILE}'); }} body {{ font-family: 'THSarabunNew'; font-size: 16pt; }} .header {{ text-align: center; position: relative; }} .logo {{ position: absolute; left: 0; width: 60px; }} .box {{ border: 1px solid #000; padding: 10px; min-height: 50px; }} </style></head><body><div class=\"header\">{logo_html}<div style=\"font-size: 22pt; font-weight: bold;\">สถานีตำรวจภูธรโรงเรียนโพนทองพัฒนาวิทยา</div><div>รายงานคดีสอบสวน</div><img style=\"position:absolute; right:0; width:60px;\" src=\"data:image/png;base64,{qr_b64}\"></div><hr><p><b>เลขที่:</b> {rid} | <b>วันที่:</b> {date_str}</p><div class=\"box\">{row.get('Statement','-')}</div>{img_html}</body></html>"
...
def investigation_module():
    ...
    with c_nav:
        ...
        if b_home.button("🏠 หน้าหลัก", use_container_width=True): setattr(st.session_state, 'current_dept', None); st.rerun()
        if b_logout.button("🚪 ออก", key="inv_logout", use_container_width=True): st.session_state.clear(); st.rerun()
    st.markdown("---")
    conn = st.connection("gsheets", type=GSheetsConnection)
    try:
        df_raw = conn.read(ttl="0").fillna("")
        if st.session_state.view_mode == "list":
            tab1, tab2 = st.tabs(["📋 รายการแจ้งเหตุ", "📊 สถิติ"])
            with tab1:
                search_q = st.text_input("🔍 ค้นหาเคส", key="inv_search_q")
...
def traffic_module():
    ...
    with c_nav:
        ...
        if b_h.button("🏠 หน้าหลัก", key="tra_home"): setattr(st.session_state, 'current_dept', None); st.rerun()
        if b_o.button("🚪 ออก", key="tra_logout"): st.session_state.clear(); st.rerun()
    st.markdown("---")
    ...
    if st.session_state.traffic_page == 'teacher':
        ...
        if st.session_state.current_user_pwd == "Patwit1510":
            with st.expander("⚙️ ระบบเลื่อนชั้นเรียน (Patwitnext)"):
                up_pwd = st.text_input("รหัสยืนยันการเลื่อนชั้น", type="password")
...
# ==========================================
# 4. MODULE: ANALYTICS (ฟังก์ชันใหม่ - บูรณาการ 2 ฐานข้อมูล)
# ==========================================
def analytics_module():
    ...
    with st.spinner("⏳ กำลังบูรณาการฐานข้อมูลเพื่อการวิเคราะห์..."):
        try:
            # 1. ดึงข้อมูลสอบสวน
            conn = st.connection("gsheets", type=GSheetsConnection)
            ...
            if 'ชั้น/ห้อง' in df_tra.columns:
                ...
                # จำลองการหาชั้นเรียนในคดีสอบสวน (ถ้ามีคอลัมน์ Grade)
                inv_lv = df_inv['Grade'].value_counts() if 'Grade' in df_inv.columns else df_inv['Location'].value_counts()
                ...
                # แสดงกราฟ
                st.markdown("### 🔍 สถิติเปรียบเทียบพฤติกรรมผิดระเบียบรายระดับชั้น")
                fig = px.bar(comb, x='ชั้น', y=['จราจร', 'สอบสวน'], barmode='group', color_discrete_map={'จราจร':'#1E3A8A', 'สอบสวน':'#ef4444'})
                st.plotly_chart(fig, use_container_width=True)
                
                st.error(f"🚩 **ความเสี่ยงสูงสุด:** เรื่อง **{top_risk}** (งานสอบสวน)")
                st.info("💡 **ข้อแนะนำเชิงนโยบาย:** ผลการวิเคราะห์ชี้ให้เห็นว่าระดับชั้นที่มีสถิติสอบสวนสูง มักมีความสัมพันธ์กับวินัยจราจรที่ต่ำลง")

        except Exception as e:
            st.warning("⚠️ ไม่สามารถดึงฐานข้อมูลมาแสดงผลได้ในขณะนี้ กรุณาตรวจสอบการตั้งค่า Secrets")
