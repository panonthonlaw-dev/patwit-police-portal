...
def analytics_module():
    ...
    try:
        ...
        # จำเป็นต้องมีฟังก์ชันเชื่อมต่อภายในเพื่อไม่ให้กระทบโค้ดภายนอก
        ...
        df_tra = pd.DataFrame(tra_vals[1:], columns=[f"C{i}" for i in range(len(tra_vals[0]))]) if tra_vals else pd.DataFrame()

        st.markdown("### 🔍 ผลการวิเคราะห์ระดับความเสี่ยงรายชั้นเรียน")
        
        if not df_tra.empty:
            # วิเคราะห์เปรียบเทียบระดับชั้น (C3 คือคอลัมน์ชั้นในงานจราจร)
            df_tra['Level'] = df_tra['C3'].apply(lambda x: str(x).split('/')[0] if x else "N/A")
            tra_stats = df_tra['Level'].value_counts()
            
            # วิเคราะห์คดีสอบสวน (ใช้ Location เป็นตัวแทนกลุ่มเสี่ยง)
        ...
        st.warning("ระบบกำลังบูรณาการฐานข้อมูล...")
...
def main():
    ...
    if not st.session_state.logged_in:
        ...
        if st.session_state.current_dept is None:
            ...
            st.markdown("---")
            # --- ส่วนที่เปลี่ยน: ปรับจาก 2 คอลัมน์ เป็น 3 คอลัมน์ ---
            c1, c2, c3 = st.columns(3)
            with c1:
                with st.container(border=True):
            ...
            with c2:
                with st.container(border=True):
                    ...
                    if st.button("เข้าใช้งานจราจร", use_container_width=True, type='primary', key="btn_to_tra"):
                        ...
                        st.session_state.search_results_df = None # Reset ค้นหา
                        st.rerun()
            with c3:
                with st.container(border=True):
