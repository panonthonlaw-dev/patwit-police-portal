...
def hazard_analytics_module():
    ...
    try:
        ...
        # 2. บังคับตรวจสอบคอลัมน์ (เรียกใช้ required_cols ที่คุณเพิ่งแก้ไป)
        # ก๊อปปี้ส่วน required_cols ที่คุณแก้ล่าสุดมาวางตรงนี้ได้เลย
        required_cols = ['Report_ID', 'Timestamp', 'Reporter', 'Incident_Type', 'Location', 'lat', 'lon']
        for c in required_cols:
            if c not in df_inv.columns: df_inv[c] = ""

        # --- 👇 วางส่วนตรวจสอบข้อมูลตรงนี้ 👇 ---
        st.write("### 🔍 ตรวจสอบพิกัดล่าสุดที่ได้รับจาก Google Sheets:")
        
        # กรองเอาเฉพาะคอลัมน์ที่เราต้องการดู
        check_df = df_inv[['Report_ID', 'Location', 'lat', 'lon']].tail(10)
        
        # แสดงผลเป็นตาราง
        st.dataframe(check_df, use_container_width=True)
        
        st.info("💡 หากในตารางช่อง lat และ lon เป็นค่าว่าง แสดงว่าข้อมูลยังไม่เข้า Google Sheet ครับ")
        # ---------------------------------------
