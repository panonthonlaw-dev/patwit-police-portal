...
from PIL import Image, ImageOps  # ✅ เพิ่ม ImageOps เข้ามา
...
COORD_MAP = {
    ...
    "อื่นๆ": {"lat": 16.293355029277148, "lon": 103.97368013832894}
}
...
from folium.features import DivIcon # เพิ่มบรรทัดนี้ไว้ด้านบนสุดของไฟล์ด้วยนะครับ
...
def create_hazard_map_obj(_df):
    ...
    # 3. วนลูปปักหมุดตามพิกัดอาคาร
    for loc_name, coords in COORD_MAP.items():
        if loc_name == "อื่นๆ": continue
        ...
        if count >= 15:
            ...
            m_color = '#facc15' # เหลือง
        elif 1 <= count <= 4:
            m_color = '#22c55e' # เขียว
        else: continue

        # ✅ 1. วาดวงกลมสีทึบเป็นพื้นหลัง (ขยายรัศมีเป็น 12 เพื่อให้ใส่เลขสวย)
        folium.CircleMarker(
            ...
            fill_color=m_color,
            fill_opacity=1.0,
            popup=f"📍 {loc_name}<br>จำนวน: {count} ครั้ง",
            tooltip=f"{loc_name}: {count}"
        ).add_to(m)

        # ✅ 2. ใส่ตัวเลขจำนวนเคสตรงกลางวงกลม
        folium.Marker(
            location=[coords['lat'], coords['lon']],
            icon=DivIcon(
                icon_size=(30,30),
                icon_anchor=(15,15), # ปรับให้ตัวเลขอยู่กึ่งกลางวงกลมพอดี
                html=f"""
                    <div style="
                        font-size: 10pt; 
                        color: {'black' if m_color == '#facc15' else 'white'}; 
                        font-weight: bold; 
                        display: flex; 
                        align-items: center; 
...
# Helpers
def get_thai_date_full(date_input):
    """แปลงวันที่จาก '29/12/2025' หรือ datetime object ให้เป็น '29 ธันวาคม พ.ศ. 2568'"""
    thai_months = [
    ...
    try:
        if isinstance(date_input, str):
            # แยกเฉพาะวันที่ออกมา กรณีมีเวลาติดมาด้วย (เช่น 29/12/2025 10:00)
            clean_date = date_input.split(' ')[0]
        ...
        return date_input # หากแปลงไม่ได้ให้คืนค่าเดิม
...
def process_image(img_file):
    ...
    try:
        ...
        # คำสั่งนี้จะเช็คว่ารูปถ่ายมาแนวไหน แล้วหมุนพิกเซลให้ถูกต้องตามแนวนั้นจริงๆ
        img = ImageOps.exif_transpose(img)
        
        # 3. แปลงเป็น RGB (เพื่อความปลอดภัยในการเซฟเป็น JPEG)
        img = img.convert('RGB')
        
        # 4. ปรับขนาดรูป (Thumbnail)
...
# --- ย้ายกลุ่มฟังก์ชันนี้มาชิดซ้ายสุดของไฟล์ (ไม่ต้องมีช่องว่างข้างหน้า) ---
...
# แก้ไขบรรทัดแรกของฟังก์ชัน ให้รับค่า appointment_info เพิ่ม
def create_summon_pdf(row, appointment_info):
    rid = str(row.get('Report_ID', ''))
    accused_name = str(row.get('Accused', ''))
    incident_type = str(row.get('Incident_Type', '-'))
    
    # 🚩 แปลงวันที่เกิดเหตุเป็นเดือนเต็ม
    raw_incident_date = str(row.get('Timestamp', '-'))
    incident_date_th = get_thai_date_full(raw_incident_date)
    ...
    # 🚩 แปลงวันที่ออกหมาย (วันนี้) เป็นเดือนเต็ม
    today_th = get_thai_date_full(get_now_th())
    ...
    <html><head><style>
        ...
        <div class="header">หนังสือเรียกสอบถาม (ส่วนงานตำรวจนักเรียน)</div>
        <div style="text-align: right;">สถานีตำรวจภูธรโรงเรียนโพนทองพัฒนาวิทยา</div>
        <div style="text-align: right;">วันที่ {today_th}</div>
        
        <p><b>เรื่อง:</b> ขอเชิญพบเพื่อสอบถามเหตุการณ์</p>
        <p><b>เรียน:</b> {accused_name} (ผู้เกี่ยวข้อง)</p>
        
        <div style="text-align: justify;">
            <p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;ด้วยทางงานป้องกันและปราบปราม(สถานีตำรวจนักเรียน) ได้รับแจ้งเหตุ <b>{incident_type}</b> ซึ่งเกิดขึ้นเมื่อวันที่ <b>{incident_date_th}</b> ตามเลขแจ้งเหตุที่ <b>{rid}</b></p>
            ...
            ณ ห้องปกครอง  เพื่อดำเนินการสอบถามข้อเท็จจริงในกรณี: <i>{incident_details[:100]}...</i> ให้เกิดความเป็นธรรมแก่ทุกฝ่าย</p>
        </div>

        <p style="margin-top: 20px;">จึงเรียนเชิญมาพบตำรวจนักเรียนตามกำหนดเวลา</p>
...
# --- [ส่วนที่เพิ่ม: การ์ดสถิติสรุปภาพรวม (Metric Cards)] ---
        ...
        # ==========================================
        # 📂 [ส่วนแสดงรายการแบบจำสถานะหน้า - Fix Pagination]
        # ==========================================
        if st.session_state.view_mode == "list":
            ...
            if c_btn_clear.button("❌ ล้าง", use_container_width=True): 
                st.rerun()
            ...
            # 🚩 [จุดสำคัญ] เปลี่ยน st.tabs เป็นการใช้ Radio สไตล์ Tab เพื่อให้จำหน้าได้
            st.markdown("""<style> div[data-testid="stHorizontalBlock"] > div:has(div.stRadio) { background: #f1f5f9; padding: 5px; border-radius: 10px; } </style>""", unsafe_allow_html=True)
            
            # สร้างตัวเลือก Tab และเก็บไว้ใน Session State (active_inv_tab)
            menu_options = [f"⚡ เคสปัจจุบัน ({len(df_active)})", f"📂 คลังคดีเก่า ({len(df_archive)})", "📊 สถิติรวม"]
            sel_tab = st.radio("เมนูนำทาง", menu_options, horizontal=True, label_visibility="collapsed", key="active_inv_tab")

            # --- [แสดงเนื้อหาตามเมนูที่เลือก] ---
            
            if "⚡ เคสปัจจุบัน" in sel_tab:
                st.markdown(f"<h4 style='color:#1E3A8A;'>⏳ รายการที่ต้องจัดการ</h4>", unsafe_allow_html=True)
                if df_active.empty:
                    ...
                    for i, row in df_active.iloc[start_p:end_p].iterrows():
                        ...
                        color = "#dc2626" if st_text == "รอดำเนินการ" else "#2563eb"
                        with cc4: st.markdown(f"<span style='color:{color}; font-weight:bold'>{st_text}</span>", unsafe_allow_html=True)
                        st.divider()
                    
                    if tot_p > 1:
                        cp1, cp2, cp3 = st.columns([1, 2, 1])
                        if cp1.button("⬅️ ย้อนกลับ", disabled=st.session_state.page_pending==1, key="btn_p_prev"): 
                        ...
                        if cp3.button("ถัดไป ➡️", disabled=st.session_state.page_pending==tot_p, key="btn_p_next"): 
                            st.session_state.page_pending+=1; st.rerun()

            elif "📂 คลังคดีเก่า" in sel_tab:
                st.markdown(f"<h4 style='color:#2e7d32;'>✅ คดีที่ยุติแล้ว/เก็บเข้าคลัง</h4>", unsafe_allow_html=True)
                if df_archive.empty:
...
# --- [ส่วนที่ 1: ระบบเช็คสิทธิ์แก้ไข] ---
                ...
                # --- [ ส่วนเดิม: ปุ่มดาวน์โหลดสำนวนคดี ] ---
                try:
                ...
                # ==========================================
                # ⚖️ [ระบบออกหมายเรียก - ปุ่มโชว์ตลอดเวลา]
                # ==========================================
                user_role = st.session_state.user_info.get('role')
                case_status = clean_val(row['Status'])
                ...
                if user_role in ["admin", "super_admin"]:
                    if case_status == "อยู่ระหว่างการดำเนินการ":
                        ...
                        appt_val = st.text_input(
                            "📅 ระบุวันและเวลานัดหมาย:", 
                            placeholder="เช่น วันจันทร์ที่ 5 มกราคม 2569 เวลา 09.00 น.",
                            key=f"input_appt_{sid}"
                        )
                        ...
                        if not appt_val:
                            ...
                            # กรณี "กรอกข้อมูลแล้ว" -> แสดงปุ่มดาวน์โหลด PDF จริง
                            try:
                                ...
                                st.success(f"✅ ข้อมูลนัดหมายวันที่ {appt_val} พร้อมออกเอกสารแล้ว")
                            except Exception as e:
                                st.error(f"❌ ไม่สามารถสร้างหมายเรียกได้: {e}")
                    
                    else:
                        # แสดงคำอธิบายถ้าสถานะยังไม่ถูกต้อง
                        st.caption(f"ℹ️ ระบบหมายเรียกจะปรากฏเมื่อสถานะคือ 'อยู่ระหว่างการดำเนินการ' (ปัจจุบัน: {case_status})")
                # ==========================================

    # 🚩 บรรทัดนี้ต้องอยู่ "หลังสุด" ของฟังก์ชันเสมอ
    except Exception as e:
...
def traffic_module():
    ...
    if st.session_state.traffic_page == 'teacher':
       ...
       # --- [ส่วนที่แก้ไข: ระบบจัดการเลื่อนชั้นเรียน สำหรับ Super Admin] ---
        if st.session_state.user_info.get("role") == "super_admin":
            with st.expander("⚙️ ระบบจัดการเลื่อนชั้นเรียน (Super Admin Only)"):
                ...
                # ช่องกรอกรหัสยืนยัน (ดึงค่าจาก UPGRADE_PASSWORD ใน Secrets)
                up_pwd = st.text_input("รหัสยืนยันการเลื่อนชั้น", type="password", key="prom_pwd_final")
                
                if st.button("ตกลงเลื่อนชั้นเรียนทั้งหมด", use_container_width=True, type="primary"):
...
def monitor_center_module():
    # --- 1. เตรียม State (Section 1) ---
    if "last_row_count" not in st.session_state:
        st.session_state.last_row_count = 0
    is_new_alert = False 

    # --- 2. CSS & JavaScript (Section 2) ---
    # ส่วนนี้คือส่วนที่คุณครูส่งมาล่าสุด ผมรวมไว้ให้ในชุดนี้แล้วครับ
    st.markdown("""
        <script>
        ...
        <style>
            ...
            .new-incident-active { animation: pulse_soft 1.5s ease-in-out infinite !important; border-left: 6px solid #dc2626 !important; background-color: #fff1f2 !important; }
            .fs-button { display: flex; align-items: center; justify-content: center; width: 100%; height: 42px; background-color: #1e293b; color: white; border-radius: 8px; border: none; cursor: pointer; font-weight: bold; font-size: 0.9em; transition: background 0.2s; }
            .fs-button:hover { background-color: #334155; }
            .alert-card-minimal { background-color: white; color: #1e293b; padding: 10px; border-radius: 10px; border: 1px solid #e2e8f0; border-left: 5px solid #ef4444; box-shadow: 0 2px 6px rgba(0,0,0,0.05); height: 100%; min-height: 90px; }
            ...
            .incident-card { padding: 15px; border-radius: 10px; margin: 10px; background: white; border: 1px solid #e2e8f0; }
            .card-new { border-left: 8px solid #dc2626 !important; }
            .card-progress { border-left: 6px solid #3b82f6 !important; background-color: #eff6ff !important; }
            .card-done { border-left: 6px solid #22c55e !important; background-color: #f0fdf4 !important; }
            .header-badge { padding: 12px; border-radius: 8px; text-align: center; font-weight: bold; margin-bottom: 10px; color: white; }
        </style>
    """, unsafe_allow_html=True)

    # --- 3. ส่วนเนื้อหาที่สั่งวาดหน้าจอ (Section 3: วางต่อจาก CSS ทันที) ---
    # ใช้ Placeholder เพื่อให้หน้าจอนิ่ง ไม่วูบวาบ (แก้ Ghosting)
    ...
    try:
        ...
        # วาด Header ลงในพื้นที่ที่จองไว้
        with header_area.container():
            c_fs, c_back, c_title = st.columns([1.5, 1.5, 7])
            c_fs.markdown('<button onclick="toggleFullScreen()" class="fs-button">🖥️ เต็มจอ</button>', unsafe_allow_html=True)
            if c_back.button("🏠 กลับเมนู", use_container_width=True):
                st.session_state.current_dept = None
                st.rerun()
            with c_title:
                st.markdown(f"<h3 style='text-align:right; margin:0;'>📡 War Room | {now_th.strftime('%H:%M:%S')}</h3>", unsafe_allow_html=True)
        ...
        if not df_raw.empty:
            ...
            # วาดเนื้อหาหลัก (3 คอลัมน์) ลงในพื้นที่ที่จองไว้
            with main_area.container():
                ...
                with col1:
                    ...
                    cards = "".join([f'<div class="incident-card card-new {"new-incident-active" if is_new_alert and i==0 else ""}"><b>🆔 {r["Report_ID"]}</b><br>📍 {r["Location"]}<br>{r["Incident_Type"]}</div>' for i, r in enumerate(df_new.values)])
                ...
                with col2:
                    ...
                    for _, r in df_prog.iterrows():
                        st.markdown(f'<div class="incident-card card-progress"><b>📝 {r["Report_ID"]}</b><br>📍 {r["Location"]}<br><small>ผู้ระงับเหตุ: {r["Student_Police_Investigator"]}</small></div>', unsafe_allow_html=True)
