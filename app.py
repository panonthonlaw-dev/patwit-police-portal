...
def hazard_analytics_module():
    ...
    try:
        ...
        if not df_map.empty:
            st.success(f"✅ ตรวจพบข้อมูลพิกัด {len(df_map)} รายการ กำลังแสดงผล...")
            
            # --- แก้ไขบรรทัดนี้: ระบุคอลัมน์ให้ชัดเจนเพื่อป้องกัน Error 'array' ---
            st.map(df_map, latitude='lat', longitude='lon') 
            
        else:
            st.warning("📍 ระบบเชื่อมต่อคอลัมน์ได้แล้ว แต่ข้อมูลใน 5 แถวท้ายยังเป็น None (ตามตารางด้านบน)")
