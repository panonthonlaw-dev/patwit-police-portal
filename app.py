...
def traffic_module():
    ...
    def create_pdf_tra(vals, img_url1, img_url2, face_url=None, printed_by="ระบบอัตโนมัติ"):
        ...
        def draw_img(url, x, y, w, h):
            try:
                if url:
                    res = requests.get(get_img_link(url), timeout=5); img_data = ImageReader(io.BytesIO(res.content))
...
def analytics_module():
    ...
    try:
        ...
        # 2. วิเคราะห์รายชั้น (จราจร vs สอบสวน)
        if 'ชั้น/ห้อง' in df_tra.columns:
            df_tra['Grade_Only'] = df_tra['ชั้น/ห้อง'].apply(lambda x: str(x).split('/')[0])
            tra_lv = df_tra['Grade_Only'].value_counts()
            # พยายามหาชั้นเรียนในสอบสวน (ถ้ามี)
            inv_lv = df_inv['Location'].value_counts() # จำลองใช้สถานที่แทนถ้าไม่มีคอลัมน์ชั้นเรียนตรงๆ
            
            comb = pd.DataFrame({'จราจร': tra_lv, 'สอบสวน': inv_lv}).fillna(0).reset_index().rename(columns={'index': 'กลุ่ม'})
            comb = comb[comb['กลุ่ม'].str.contains("ม.", na=False)].sort_values('กลุ่ม')
            
            st.plotly_chart(px.bar(comb, x='กลุ่ม', y=['จราจร', 'สอบสวน'], barmode='group', title="สถิติการทำผิดระเบียบรายระดับชั้น"), use_container_width=True)

            st.error(f"🚩 **ความเสี่ยงสูงสุดในขณะนี้:** คดีประเภท **{top_risk}**")
            st.info("💡 **มาตรการแนะนำ:** ควรกวดขันวินัยในระดับชั้นที่มีสถิติการทำผิดกฎจราจรและคดีสอบสวนรวมกันสูงที่สุด")
    except:
        st.warning("ระบบกำลังรอฐานข้อมูลเพื่อประมวลผลการวิเคราะห์สถิติขั้นสูง...")
