...
def hazard_analytics_module():
    ...
    try:
        ...
        st.write(f"🔍 กำลังตรวจสอบข้อมูลจาก: `{target_sheet}`")

        # 2. อ่านข้อมูลสด (ห้ามใช้ Cache)
        df_inv = conn.read(worksheet=target_sheet, ttl=0)
        
        # --- [จุดสำคัญ] บังคับล้างช่องว่างที่หัวตาราง ---
        df_inv.columns = [str(c).strip() for c in df_inv.columns]

        # 3. บังคับสร้างคอลัมน์ lat, lon ถ้ายังหาไม่เจอ (เพื่อป้องกันแอปค้าง)
        if 'lat' not in df_inv.columns: df_inv['lat'] = ""
        if 'lon' not in df_inv.columns: df_inv['lon'] = ""

        # --- 👇 ส่วนตรวจสอบข้อมูลที่แก้ไขใหม่ 👇 ---
        st.write("### 🔍 ตรวจสอบพิกัดล่าสุดที่ได้รับจาก Google Sheets:")
        
        # เลือกเฉพาะคอลัมน์ที่มีอยู่จริง เพื่อป้องกัน Error 'KeyNotFound'
        available_cols = [c for c in ['Report_ID', 'Location', 'lat', 'lon'] if c in df_inv.columns]
        check_df = df_inv[available_cols].tail(10)
        
        # แสดงผลตาราง
        st.dataframe(check_df, use_container_width=True)
        
        # ตรวจสอบว่าในตารางมีข้อมูลพิกัดจริงๆ หรือไม่
        has_data = df_inv[df_inv['lat'] != ""].shape[0] > 0
        if not has_data:
            st.warning("⚠️ พบคอลัมน์แล้ว แต่ 'ทุกแถว' ใน Google Sheet (ช่อง R และ S) ยังไม่มีตัวเลขพิกัด")
            st.info("💡 วิธีแก้: ลองพิมพ์ตัวเลขเช่น 16.2941 ในช่อง R2 และ 103.9782 ในช่อง S2 ของ Google Sheet เพื่อทดสอบ")
        else:
            st.success(f"✅ ตรวจพบข้อมูลพิกัดแล้ว {df_inv[df_inv['lat'] != ''].shape[0]} รายการ")

    except Exception as e:
        # ถ้ายังขึ้น Error สีแดง ให้โชว์สาเหตุที่แท้จริงออกมาเลย
        st.error(f"❌ ระบบขัดข้องเนื่องจาก: {e}")
        st.write("รายชื่อคอลัมน์ที่ระบบมองเห็นตอนนี้:", df_inv.columns.tolist() if 'df_inv' in locals() else "ไม่สามารถโหลดตารางได้")
#--------------------
# PDF & Chart Libraries
try:
