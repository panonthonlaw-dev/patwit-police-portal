...
# แก้ไขฟังก์ชันดึงรูปภาพเพื่อเลี่ยง MediaFileStorageError
        ...
        if st.session_state.view_mode == "list":
            ...
            with tab_dash:
                ...
                if tc > 0:
                    ...
                    with col1: st.markdown("**🔹 ประเภทเหตุ**"); st.bar_chart(df_display['Incident_Type'].value_counts())
                    with col2: st.markdown("**🔹 สถานที่เกิดเหตุ**"); st.bar_chart(df_display['Location'].value_counts())
            ...
            if not sel.empty:
                ...
                with st.form("full_inv_form"):
                    ...
                    if st.form_submit_button("💾 บันทึกข้อมูล") and not is_lock:
                        ...
                        df_raw.at[idx_raw, 'Teacher_Investigator'] = v_tea; df_raw.at[idx_raw, 'Student_Police_Investigator'] = v_stu
                ...
                st.download_button(label="📥 ดาวน์โหลด PDF (สำนวนคดี)", data=create_pdf_inv(row), file_name=f"Report_{sid}.pdf", mime="application/pdf", use_container_width=True)
    except Exception as e: st.error(f"Error: {e}")

# ==========================================
# 3. MODULE: TRAFFIC (คงเดิม 100% + แก้รูปภาพ)
# ==========================================
def traffic_module():
    user = st.session_state.user_info
    ...
    def upload_to_drive(file_obj, filename):
        file_content = file_obj.getvalue(); base64_str = base64.b64encode(file_content).decode('utf-8')
    ...
    def create_pdf_tra(vals, img_url1, img_url2, face_url=None, printed_by="ระบบอัตโนมัติ"):
        ...
        if os.path.exists(FONT_FILE):
            pdfmetrics.registerFont(TTFont('Thai', FONT_FILE)); pdfmetrics.registerFont(TTFont('ThaiBold', FONT_BOLD if os.path.exists(FONT_BOLD) else FONT_FILE))
            fn, fb = 'Thai', 'ThaiBold'
        ...
        name, std_id, cl, br, co, pl = str(vals[1]), str(vals[2]), str(vals[3]), str(vals[4]), str(vals[5]), str(vals[6])
        lic_s, tax_s, hel_s = str(vals[7]), str(vals[8]), str(vals[9])
        raw_note = str(vals[12]).strip() if len(vals) > 12 else ""; note_text = raw_note if raw_note and raw_note.lower() != "nan" else "ไม่พบประวัติ"
        score = str(vals[13]) if len(vals) > 13 and str(vals[13]).lower() != "nan" else "100"
        c.setFont(fn, 16); c.drawString(60, height - 115, f"ชื่อ-นามสกุล: {name}"); c.drawString(300, height - 115, f"ยี่ห้อรถ: {br}")
        c.drawString(60, height - 135, f"รหัสนักเรียน: {std_id}"); c.drawString(300, height - 135, f"สีรถ: {co}")
        c.drawString(60, height - 155, f"ระดับชั้น: {cl}"); c.setFont(fb, 16); c.drawString(300, height - 155, f"ทะเบียน: {pl}")
        c.setFont(fb, 18); color_val = (0.7, 0, 0) if int(score) < 80 else (0, 0.5, 0); c.setFillColorRGB(*color_val); c.drawString(60, height - 185, f"คะแนนความประพฤติจราจรคงเหลือ: {score} คะแนน"); c.setFillColorRGB(0, 0, 0)
        ...
        u1, u2, uf = get_img_link(img_url1), get_img_link(img_url2), get_img_link(face_url)
        draw_img(u1, 70, height - 415, 180, 180); draw_img(u2, 300, height - 415, 180, 180)
        if uf: draw_img(uf, 450, height - 200, 90, 110)
    ...
    if st.session_state.traffic_page == 'teacher':
        ...
        if c_btn_search.button("ค้นหา", type="primary", use_container_width=True) or q:
            if st.session_state.df_tra is not None:
                df = st.session_state.df_tra; st.session_state.search_results_df = df[df.iloc[:, [1, 2, 6]].apply(lambda r: r.astype(str).str.contains(q, case=False).any(), axis=1)]
        if c_btn_clear.button("ล้างค่า", use_container_width=True): st.session_state.search_results_df = None; st.session_state.df_tra = None; st.rerun()
        ...
        f_lv = col_f2.selectbox("📚 ระดับชั้น:", ["ทั้งหมด"] + u_lv); f_br = col_f3.selectbox("🏍️ ยี่ห้อรถ:", ["ทั้งหมด"] + u_br)
            ...
            if tdf.empty: st.warning("❌ ไม่พบข้อมูล")
                ...
                for i, row in tdf.iterrows():
                    ...
                    with st.expander(f"📍 {v[6]} | {v[1]}"):
                        ...
                        imgs = [get_img_link(v[14]), get_img_link(v[10]), get_img_link(v[11])]
                        valid_imgs = [img for img in imgs if img]; st.image(valid_imgs, width=200)
                        if st.session_state.officer_role == "admin":
                            col_act1, col_act2 = st.columns(2)
                            col_act1.download_button("📥 โหลด PDF", create_pdf_tra(v, v[10], v[11], v[14], st.session_state.officer_name), f"{v[6]}.pdf", use_container_width=True)
                            ...
                            with st.form(key=f"sc_form_{i}"):
                                ...
                                if c_sub1.form_submit_button("🔴 หักแต้ม", use_container_width=True) and nt and pwd == st.session_state.current_user_pwd:
                                    s = connect_gsheet_universal(); cell = s.find(str(v[2])); ns = max(0, sc-pts); tn = get_now_th().strftime('%d/%m/%Y %H:%M')
                                    old = str(v[12]).strip() if str(v[12]).lower()!="nan" else ""; new_log = f"{old}\n[{tn}] หัก {pts} คะแนน: {nt} (โดย: {st.session_state.officer_name})"
                                    s.update(f'M{cell.row}:N{cell.row}', [[new_log, str(ns)]]); st.success("บันทึกแล้ว"); load_tra_data(); st.rerun()
        
        st.markdown("---")
        if st.session_state.current_user_pwd == "Patwit1510":
            with st.expander("⚙️ ระบบจัดการเลื่อนชั้นเรียน (Super Admin Only)"):
                st.warning("⚠️ คำเตือน:ระบบจะเป็นการแก้ไขถาวรไม่สามารถย้อนกลับได้กรุณาระมัดระวัง")
        ...
        with st.form("edit_form"):
            nm = st.text_input("ชื่อ", v[1]); cl = st.text_input("ชั้น", v[3]); br = st.selectbox("ยี่ห้อ", ["Honda", "Yamaha", "Suzuki", "GPX", "Kawasaki", "อื่นๆ"]); pl = st.text_input("ทะเบียน", v[6])
            lc = st.radio("ใบขับขี่", ["✅ มี", "❌ ไม่มี"], index=0 if "มี" in v[7] else 1, horizontal=True)
            if st.form_submit_button("บันทึก", type="primary", use_container_width=True):
                ...
                sheet.update(f'B{cell.row}:L{cell.row}', [[nm, v[2], cl, br, v[5], pl, lc, v[8], v[9], v[10], v[11]]])
                load_tra_data(); st.success("เสร็จสิ้น"); st.session_state.traffic_page = 'teacher'; st.rerun()
        if st.button("ยกเลิก"): st.session_state.traffic_page = 'teacher'; st.rerun()
        ...
        if st.session_state.df_tra is not None:
            ...
            c1, c2 = st.columns(2); c1.plotly_chart(px.pie(df, names='Col_7', title="ใบขับขี่")); c2.plotly_chart(px.pie(df, names='Col_9', title="หมวก"))

# ==========================================
# 4. MODULE: BEHAVIORAL ANALYTICS (ฟังก์ชันวิเคราะห์ใหม่ - Read Only)
# ==========================================
def analytics_module():
    ...
    try:
        ...
        def connect_gsheet_universal(): # Sub-function for Analytics
            if "connections" in st.secrets:
                creds_dict = dict(st.secrets["connections"]["gsheets"]); scope = ["https://spreadsheets.google.com/feeds", "https://www.googleapis.com/auth/drive"]
                creds = ServiceAccountCredentials.from_json_keyfile_dict(creds_dict, scope); return gspread.authorize(creds).open(SHEET_NAME_TRAFFIC).sheet1
        ...
        st.markdown("### 🔍 สรุปจุดเสี่ยงรายระดับชั้น (Integrated Risk Assessment)")
        ...
        if 'ชั้น/ห้อง' in df_tra.columns:
            ...
            st.error(f"🚩 **ความเสี่ยงสูงสุด:** คดีประเภท **{top_risk}**"); st.warning(f"🛡️ **ข้อเสนอแนะ:** ควรกวดขันวินัยในระดับชั้นที่มีสถิติรวมสูงกว่าค่าเฉลี่ย")
    ...
    if not st.session_state.logged_in:
        ...
        with col:
            ...
            with st.container(border=True):
                ...
                st.markdown("<h3 style='text-align:center;'>ศูนย์ปฏิบัติการกลาง<br>โรงเรียนโพนทองพัฒนาวิทยา</h3>", unsafe_allow_html=True)
