...
# ==========================================
# 3. MODULE: TRAFFIC (เพิ่มเติมสถิติภาพรวมและสถานะ)
# ==========================================
def traffic_module():
    user = st.session_state.user_info
                        ...
                        if st.session_state.officer_role == "admin":
                            with st.form(key=f"sc_form_{i}"):
                                ...
                                if st.form_submit_button("🔴 หักแต้ม") and nt and pwd == st.session_state.current_user_pwd:
                                    sheet = connect_gsheet_universal()
                                    if sheet:
                                        cell = sheet.find(str(v[2])); ns = max(0, sc-pts); action = "หัก" if deduct else "เพิ่ม"; tn = get_now_th().strftime('%d/%m/%Y %H:%M'); old_log = str(v[12]).strip() if str(v[12]).lower()!="nan" else ""; new_log = f"{old_log}\n[{tn}] {action} {pts} คะแนน: {nt}"; sheet.update(f'M{cell.row}:N{cell.row}', [[new_log, str(ns)]]); st.success("บันทึกแล้ว"); st.session_state.df_tra = None; st.rerun()
...
# ==========================================
# 4. MODULE: ANALYTICS (หัวข้อไว้ก่อน)
# ==========================================
def analytics_module():
    st.header("📊 วิเคราะห์พฤติกรรม")
    st.info("💡 ระบบกำลังรวบรวมพฤติกรรมย้อนหลังเพื่อวิเคราะห์เชิงลึก")
