...
def create_hazard_map_obj(_df):
    ...
    # 1. กรองเอาเฉพาะรายการที่มีใน COORD_MAP และไม่ใช่ "อื่นๆ" ออกมาก่อน
    # เพื่อให้การนับความถี่ (Risk) แม่นยำเฉพาะจุดที่เราจะปักหมุด
    df_filtered = _df[_df['Location'].isin(COORD_MAP.keys())].copy()
    df_filtered = df_filtered[df_filtered['Location'] != "อื่นๆ"]
    
    # 2. นับจำนวนเหตุการณ์รายสถานที่
    location_counts = df_filtered['Location'].value_counts().to_dict()
    ...
    # 4. วนลูปปักหมุดจากข้อมูลที่กรองแล้ว
    for _, row in df_filtered.iterrows():
        loc_name = str(row.get('Location', '')).strip()
        coords = COORD_MAP.get(loc_name)
        
        # ดึงค่าความถี่จาก Dictionary ที่เรานับไว้
        count = location_counts.get(loc_name, 0)
        marker_color = get_risk_color(count)
        
        # Jitter (เขย่าจุดเล็กน้อย)
        ...
        folium.CircleMarker(
            ...
            weight=2,
            fill=True, 
            fill_color=marker_color, # บังคับใช้สีตามที่เราคำนวณ
            fill_opacity=1.0,
