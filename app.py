    ...
    if st.session_state.traffic_page == 'teacher':
        ...
        if st.session_state.current_user_pwd == "Patwit1510":
            with st.expander("⚙️ ระบบจัดการเลื่อนชั้นเรียน (Super Admin Only)"):
                st.warning("⚠️ คำเตือน:ระบบจะเป็นการแก้ไขถาวรไม่สามารถย้อนกลับได้"); up_pwd = st.text_input("รหัสเลื่อนชั้น", type="password", key="prom_pwd")
        ...
        if st.session_state.df_tra is not None:
            ...
            with c1:
                st.plotly_chart(px.pie(df, names='Col_7', title="ใบขับขี่", hole=0.3), use_container_width=True)
            with c2:
