...
# --- [ส่วนที่เพิ่ม: การ์ดสถิติสรุปภาพรวม (Metric Cards)] ---
        ...
        if st.session_state.view_mode == "list":
            ...
            if not sel.empty:
               ...
               # เปลี่ยนมาเช็คว่าถ้าเป็น super_admin ให้ปลดล็อกอัตโนมัติ หรือถามรหัสยืนยัน Patwitsafe (ตัวแปร UPGRADE_PASSWORD ใหม่)
if is_lock and cur_sta == "ดำเนินการเรียบร้อย":
    if st.session_state.user_info.get('role') == 'super_admin':
        # ถ้าเป็นแอดมินสูงสุด ให้เห็นปุ่มกดปลดล็อกได้เลยไม่ต้องพิมพ์รหัส
        if st.button("🔓 ปลดล็อคแก้ไข (สิทธิ์แอดมินสูงสุด)"):
