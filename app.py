...
def hazard_analytics_module():
    # 1. ข้อมูลพิกัดที่คุณเตรียมไว้ (ย้ายมาไว้ในฟังก์ชันหรือนอกฟังก์ชันก็ได้)
    COORD_MAP = {
    ...
    st.markdown("<h2 style='text-align: center; color: #1E3A8A;'>📍 แผนที่จุดเสี่ยงภายในโรงเรียน</h2>", unsafe_allow_html=True)

    try:
        conn = st.connection("gsheets", type=GSheetsConnection)
        target_sheet = get_target_sheet_name()

        # ✅ แก้ไขจุดที่ 1: ตั้งค่า ttl=300 (5 นาที) เพื่อลดความถี่ในการอ่าน Sheets
        # ข้อมูลจะถูกเก็บใน Cache 5 นาที ไม่ต้องส่งคำขอใหม่ทุกครั้งที่หน้าจอขยับ
        df_raw = conn.read(worksheet=target_sheet, ttl=300)
        df_inv = pd.DataFrame(df_raw)

        if not df_inv.empty:
            # ✅ แก้ไขจุดที่ 2: แปลงสถานที่ให้เป็นพิกัดตาม COORD_MAP
            def get_fixed_coord(loc_name, axis):
                loc_clean = str(loc_name).strip()
                # ถ้าไม่เจอสถานที่ในตาราง ให้ไปปักที่จุด "อื่นๆ"
                res = COORD_MAP.get(loc_clean, COORD_MAP["อื่นๆ"])
                return res[axis]

            df_inv['fixed_lat'] = df_inv['Location'].apply(lambda x: get_fixed_coord(x, 'lat'))
            df_inv['fixed_lon'] = df_inv['Location'].apply(lambda x: get_fixed_coord(x, 'lon'))

            # สร้างแผนที่ Folium
            school_center = [16.293596, 103.972502]
            ...
            # ใส่จุดลงบนแผนที่ (ใช้ MarkerCluster เพื่อความสวยงาม)
            marker_cluster = MarkerCluster().add_to(m)
            for _, row in df_inv.iterrows():
                # ใส่ Jitter เล็กน้อยเพื่อให้จุดไม่ทับกันสนิทในอาคารเดียวกัน
                j_lat = row['fixed_lat'] + random.uniform(-0.00003, 0.00003)
                j_lon = row['fixed_lon'] + random.uniform(-0.00003, 0.00003)
                
                folium.CircleMarker(
                    location=[j_lat, j_lon],
                    radius=7, color='white', weight=1, fill=True,
                    fill_color='#ef4444', fill_opacity=0.8,
                    popup=f"<b>{row['Location']}</b><br>ID: {row['Report_ID']}<br>เหตุ: {row['Incident_Type']}"
                ).add_to(marker_cluster)

            # ✅ แก้ไขจุดที่ 3: แสดงแผนที่โดย "ไม่ต้อง" มี time.sleep หรือ rerun วนลูป
            st_folium(m, width="100%", height=600, returned_objects=[])
            
            st.success("✅ แสดงผลตามตำแหน่งอาคารในโรงเรียนเรียบร้อย (อัปเดตข้อมูลทุก 5 นาที)")
        else:
            st.warning("ยังไม่มีข้อมูลการแจ้งเหตุ")

    except Exception as e:
        # หากยังติด Quota ให้แสดงปุ่มกด Refresh ด้วยมือแทน
        if "429" in str(e):
            st.error("⚠️ คำขออ่านข้อมูลเต็มโควต้า (Read Quota Exceeded)")
            st.info("กรุณารอประมาณ 1-2 นาที แล้วกดปุ่มด้านล่างเพื่อลองอีกครั้ง")
            if st.button("🔄 ลองดึงข้อมูลใหม่"):
                st.rerun()
        else:
