...
# --- [ส่วนที่เพิ่ม: การ์ดสถิติสรุปภาพรวม (Metric Cards)] ---
        ...
        if st.session_state.view_mode == "list":
            ...
            if not sel.empty:
                ...
                # --- [ส่วนที่แก้ไข: ระบบล็อกตามระดับสิทธิ์] ---
                cur_sta = clean_val(row['Status'])
                user_role = st.session_state.user_info.get('role')

                # ตั้งค่าล็อกเริ่มต้น: ล็อกไว้ก่อน
                is_lock = True 

                if user_role == "super_admin":
                    if cur_sta == "ดำเนินการเรียบร้อย":
                        # ถ้าคดีจบแล้ว ต้องกดปุ่มยืนยันก่อน
                        ...
                        # คดียังไม่จบ Super Admin แก้ได้ทันที
                        is_lock = False

                elif user_role == "admin":
                    if cur_sta != "ดำเนินการเรียบร้อย":
                        # ครูทั่วไปแก้ไขได้เฉพาะคดีที่ยังไม่ปิด
                        is_lock = False 
                    else:
                        # ถ้าคดีจบแล้ว ครูทั่วไปถูกล็อกถาวร
