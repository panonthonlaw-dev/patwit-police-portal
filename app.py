...
def monitor_center_module():
    ...
    # 2. CSS Styles (รองรับทั้งแบบนิ่งและแบบเลื่อน)
    st.markdown("""
        <style>
            ::-webkit-scrollbar { width: 0px; background: transparent; }
            
            /* --- Animation สำหรับการ์ดใหม่ (กะพริบ) --- */
            ...
            /* --- Animation สำหรับเลื่อนขึ้น (End Credit) --- */
            @keyframes scroll-up {
                0% { transform: translateY(0%); }
                100% { transform: translateY(-50%); } 
            }

            /* Container สำหรับโซนเลื่อน */
            .scroll-window {
                height: 70vh; /* ความสูงของจอที่จะแสดง */
                overflow: hidden;
                position: relative;
                padding: 5px;
            }
            .scroll-content {
                position: absolute;
                width: 100%;
                /* เวลาในการเลื่อน 60s (ปรับเลขนี้ถ้าอยากให้เร็ว/ช้า) */
                animation: scroll-up 60s linear infinite; 
            }
            .scroll-window:hover .scroll-content {
                animation-play-state: paused; /* เอาเมาส์ชี้แล้วหยุด */
            }
            ...
            .case-id-badge {
                ...
                background: rgba(255,255,255,0.8); padding: 2px 6px; border-radius: 4px; float: right;
            }
        </style>
        <div style="text-align:center; padding:10px; border-bottom:2px solid #f1f5f9; margin-bottom:20px;">
            ...
            <p style="color:#64748b; font-size:0.8em;">(Auto-Refresh 10s | Scroll Mode เมื่อเคส > 10)</p>
        </div>
    """, unsafe_allow_html=True)
    ...
    # 4. Helper: สร้าง HTML String สำหรับโหมดเลื่อน
    def make_card_html(row, status_type):
        if status_type == "new":
            css = "card-new"; bg_time = "#b91c1c"; txt_time = "#7f1d1d"
        elif status_type == "prog":
            css = "card-progress"; bg_time = "#1e40af"; txt_time = "#1e40af"
        else:
            css = "card-done"; bg_time = "#14532d"; txt_time = "#14532d"

        responsible = row['Student_Police_Investigator'] if status_type != "new" else "-"
        
        return f"""
        <div class="incident-card {css}">
            <div style="margin-bottom:5px;">
                <span style="font-size:1.2em; font-weight:bold; color:{bg_time};">📍 {row['Location']}</span>
            </div>
            <div style="font-weight:bold; color:#1e293b; margin-bottom:5px;">{row['Incident_Type']}</div>
            <div style="font-size:0.85em; color:{txt_time}; margin-bottom:5px;">
                🕒 {row['Timestamp']}
                <span class="case-id-badge" style="color:{bg_time};">📝 {row['Report_ID']}</span>
            </div>
            <div style="font-size:0.9em; color:#475569; margin-top:5px; border-top:1px solid rgba(0,0,0,0.05); padding-top:5px;">
                {row['Details']}
            </div>
            {f'<div style="font-size:0.8em; color:#475569; margin-top:5px;">👮 {responsible}</div>' if status_type != 'new' else ''}
        </div>
        """
    ...
    try:
        ...
        if not df.empty and 'Status' in df.columns:
            ...
            # 1. รอดำเนินการ (เอาทั้งหมด ไม่จำกัด)
            df_new = df[df['Status'] == "รอดำเนินการ"].iloc[::-1]
            
            # 2. กำลังดำเนินการ (เอาทั้งหมด ไม่จำกัด)
            df_prog = df[df['Status'] == "อยู่ระหว่างการดำเนินการ"].iloc[::-1]
            
            # 3. เสร็จสิ้น (เอาแค่ 10 ล่าสุด)
            ...
            with c1:
                ...
                if df_new.empty:
                    ...
                    for _, row in df_new.iterrows():
                        st.markdown(make_card_html(row, "new"), unsafe_allow_html=True)
                else:
                    # มากกว่า 10: แสดงแบบเลื่อน (End Credit)
                    html_content = "".join([make_card_html(row, "new") for _, row in df_new.iterrows()])
                    st.markdown(f"""
            ...
            with c2:
                st.markdown('<div class="header-badge" style="background:#2563eb;">🔵 กำลังดำเนินการ (ทั้งหมด)</div>', unsafe_allow_html=True)
                ...
                if df_prog.empty:
                    ...
                    for _, row in df_prog.iterrows():
                        st.markdown(make_card_html(row, "prog"), unsafe_allow_html=True)
                else:
                    # มากกว่า 10: แสดงแบบเลื่อน (End Credit)
                    html_content = "".join([make_card_html(row, "prog") for _, row in df_prog.iterrows()])
                    st.markdown(f"""
                    <div class="scroll-window">
                        <div class="scroll-content">
                            {html_content}
            ...
            with c3:
                st.markdown('<div class="header-badge" style="background:#16a34a;">✅ เสร็จสิ้น (10 ล่าสุด)</div>', unsafe_allow_html=True)
                
                if df_done.empty:
                    st.caption("- ว่าง -")
                else:
                    # แสดงนิ่งๆ เสมอ (เพราะตัดมาแค่ 10)
                    for _, row in df_done.iterrows():
                        st.markdown(make_card_html(row, "done"), unsafe_allow_html=True)
