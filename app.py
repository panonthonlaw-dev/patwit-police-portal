...
# ==========================================
# 4. NEW MODULE: BEHAVIORAL ANALYTICS (ส่วนเพิ่มเติม)
# ==========================================
def analytics_module():
    ...
    with c_brand:
        ...
        with c_logo: 
            if LOGO_PATH: st.image(LOGO_PATH, use_column_width=True)
        with c_text:
            st.markdown(f'<div style="display: flex; flex-direction: column; justify-content: center; height: 100%;"><div style="font-size: 22px; font-weight: bold; color: #1E3A8A; line-height: 1.2;">ศูนย์ปฏิบัติการวิเคราะห์ข้อมูลพฤติกรรมศาสตร์</div><div style="font-size: 16px; color: #475569; margin-top: 4px;"><span style="font-weight: bold;">📊 ระบบวิเคราะห์เชิงลึก</span> | ผู้เข้าใช้งาน: {user["name"]}</div></div>', unsafe_allow_html=True)
    with c_nav:
        st.write("")
        st.write("")
        b_home, b_logout = st.columns(2)
        if b_home.button("🏠 หน้าหลัก", use_container_width=True, key="ana_h"): setattr(st.session_state, 'current_dept', None); st.rerun()
        if b_logout.button("🚪 ออก", key="ana_o", use_container_width=True): st.session_state.clear(); st.rerun()
    st.markdown("---")

    with st.spinner("⏳ กำลังวิเคราะห์ Big Data พฤติกรรมศาสตร์..."):
        try:
            # ดึงข้อมูลสอบสวน (Read-Only)
            conn_inv = st.connection("gsheets", type=GSheetsConnection)
            df_inv = conn_inv.read(ttl="0").fillna("")
            
            # ดึงข้อมูลจราจร (Read-Only)
            ...
            # --- วิเคราะห์ระดับชั้น ---
            st.markdown("### 🔍 สถิติเปรียบเทียบพฤติกรรมรายระดับชั้น (ม.1 - ม.6)")
            if not df_tra.empty and 'ชั้น/ห้อง' in df_tra.columns:
                ...
                inv_lv = df_inv['Location'].value_counts() # จำลองกลุ่มเสี่ยง
                ...
                st.plotly_chart(px.bar(comb, x='ชั้น', y=['งานจราจร', 'งานสอบสวน'], barmode='group', title="กราฟแสดงความถี่การพบเหตุผิดระเบียบ"), use_container_width=True)

            # --- วิเคราะห์ความเสี่ยงสูงสุด ---
            if not df_inv.empty and 'Incident_Type' in df_inv.columns:
                top_risk = df_inv['Incident_Type'].value_counts().idxmax()
                st.error(f"🚩 **ความเสี่ยงทางคดีสูงสุด:** เรื่อง **{top_risk}** (จำเป็นต้องมีการรณรงค์เชิงป้องกันเป็นพิเศษ)")
        except:
            st.warning("⚠️ อยู่ในระหว่างการเชื่อมต่อฐานข้อมูลสำหรับการวิเคราะห์เชิงลึก...")

# ==========================================
# 5. MAIN ENTRY (แก้ไขเพื่อเพิ่มปุ่มที่ 3)
# ==========================================
def main():
    ...
    if not st.session_state.logged_in:
        ...
        if st.session_state.current_dept is None:
            ...
            st.markdown("---")
            # --- ส่วนที่เปลี่ยน: จาก 2 คอลัมน์ เป็น 3 คอลัมน์ ---
            c1, c2, c3 = st.columns(3)
            with c1:
                with st.container(border=True):
